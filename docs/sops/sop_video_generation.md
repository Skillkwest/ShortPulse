...
| Kie      | `kie-ai/seedance-2`, `kie-ai/seedance-2-fast` | 16:9 default (allowed: 1:1, 21:9, 4:3, 3:4, 16:9, 9:16) | Active Seedance 2 lanes via `/api/fal/kie-seedance-2-submit                                                                                                                                                                                                                                                                                                                                                                                                                                                              | status`and`/api/fal/kie-seedance-2-fast-submit                                                                                                            | status`. Panel UX follows the Kling-pattern shot workspace and linked Character/Element surfaces, but submit compiles into Seedance-native prompt, frame, and multimodal reference payloads (`first*frame_url`, `last_frame_url`, `reference*\*\_urls`). Video settings expose duration `5/10/15`, resolution `720p/1080p`, optional audio, plus `return_last_frame`and`web_search`. |
| Kie      | `kie-ai/seedance-1.5-pro`                     | 1:1 default (allowed: 1:1, 21:9, 4:3, 3:4, 16:9, 9:16)  | Deprecated compatibility lane retained for restore/runtime continuity. New picker selection should use Seedance 2. Existing provider routes and docs remain during the compatibility window while persisted state remaps through `replacementModelId` to `kie-ai/seedance-2`. |
