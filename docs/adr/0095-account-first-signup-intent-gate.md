...
- Historical note: the first Google signup implementation remained email-first because it used the hashed-email hook match. ADR 0096 added the Google-only short-lived IP-bound fallback; ADR 0097 makes Google signup independent from the visible email/password fields.
