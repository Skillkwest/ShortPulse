...
| 5 | `Video workflow` | 4.5 | Below floor | Focused local reload proof plus approval-gated provider boundary | Video reload hydration invariants, canonical Kie/Seedance route/provider labels, and local retirement of the legacy `/api/upload-video` adapter are green, but provider/generation proof remains approval-gated and broader customer-flow proof is still thin. | Keep reload/provider-contract/canonical-route proof green and refresh bounded authenticated Video proof when safe. |
...
| 9 | `Sound workflow` | 5.8 | Below floor | Local route/helper proof | Local ElevenLabs and staging proof helps, the stale audio upload adapter is removed, and local audio references now use the browser-direct media upload route pair, but provider/env posture and authenticated output insertion are still not production-proven. | Production-safe provider/env check, then approved minimal sound output proof if needed. |
...
| 12 | `Projects and workspace restore` | 6.5 | Near floor | Prior production checked plus current local reload, API-error, and owner-boundary proof | Project restore has better evidence than many systems, and safe plain-text save/load failures, read/reset route failure observability, project service user-scope queries, list/preview enrichment user scoping, and not-owned route guards now have focused coverage, but it still needs a fresh post-convergence save/open/restore proof on a stable tree. | Reconfirm project save/open/restore on the stable lane. |
...
| 15 | `Admin and launch operations` | 6.8 | Floor with watch - deploy gate open | Docs/checks, route parity, operator route protections | Admin surfaces are not the top blocker, and route parity now fails the default retired fallback/legacy route list with focused test coverage for the default gate. Current production still exposes locally retired `/api/upload-video`, `/api/upload-audio`, and `/api/media/admit-image-asset` until the local route-retirement work is deployed; launch-week operator proof is still needed. | Deploy local route retirements, rerun strict route parity, then perform authenticated operator smoke where safe. |
| 16 | `Credits, pricing, billing, and entitlements` | 7.5 | Launchable with watch | Billing readiness `9 pass / 1 warn / 0 fail` on `2026-06-17` | Production billing catalog, offers, env contract, hidden free tier, signup trigger, and renewal fail-closed checks look good. Stripe webhook event proof remains unproven locally because `STRIPE_SECRET_KEY` is unavailable in this session. | Resolve Stripe webhook event proof boundary with the correct secret/prod-safe lane. |
...
- The queue freshness note reflects the latest `2026-06-17` production route/fail-closed refresh against deployment `shortpulse-fug3hxhsc-kirk-artmans-projects.vercel.app`; `node scripts/check_secret_exposure.js` passed; hosted production `sql/check_runtime_sql_security_audit.sql` returned `353 / 353` passing checks with `failing_checks = 0`; and local project owner-boundary coverage proves owner-scoped project list/read/update/delete service queries, preview enrichment user scoping, and not-owned item/workspace route guards. This does not lift authenticated workflow or provider proof.

The strongest areas right now are public entry/account trust, billing, security fail-closed posture, admin route/docs hygiene, and model/provider catalog hygiene. The weakest areas are recovery/settlement, media organization, right-rail reuse, AI Studio shell/navigation, and Video workflow stability.
