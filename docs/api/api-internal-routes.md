...
| `/api/internal/admin-user-health-fleet/run` | `POST`, `GET` | `x-shortpulse-cron-secret` or `Authorization: Bearer <fleet-secret>` | Run one bounded active-user fleet health scan. Persists run/snapshot/finding records, marks partial runs when time-budget or compatibility limits are hit, prunes retention history, and returns run metrics (`targeted`, `processed`, `failed`, `partial`, `durationMs`, `errors`). | `frontend/pages/api/internal/admin-user-health-fleet/run.ts`, `frontend/lib/server/adminUserHealth/fleet.ts`, `sql/configure_admin_user_health_fleet_scheduler_supabase.sql` |
