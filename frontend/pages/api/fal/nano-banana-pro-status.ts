  ...
  try {
    ...
    if (!contentType || !contentType.includes("application/json")) {
      ...
      return res.status(500).json({
        error: "Fal Nano Banana Pro returned non-JSON response",
    ...
    if (!resultContentType || !resultContentType.includes("application/json")) {
      ...
      return res.status(200).json({
        status: "error",
        state: "error",
        error: "Fal Nano Banana Pro result returned non-JSON response",
        detail: text.substring(0, 500),
        request_id: requestId,
      });
    }
    ...
    return res.status(200).json({
      status: "error",
      state: "error",
      error: "Fal Nano Banana Pro status check failed",
      detail: String(error),
      request_id: requestId,
    });
  } finally {
