...
| UI runtime reference regressions | UI `ReferenceError` / undefined symbol issues on app or admin routes that can be tied to a concrete render/runtime seam | 4 | `resolved`, `verified-existing-fix`, `noise-filtered` | `medium` | `can-handle-with-guardrails` | Repair the broken scope/prop contract, add targeted regression coverage, or filter pure dev/HMR noise without hiding real runtime bugs | runtime stack, route, code path, targeted UI/runtime test, recurrence check | escalate if the runtime path is not reproducible and current source does not explain it |
