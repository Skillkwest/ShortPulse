...
- Client route-transition failures (`client.route_change`, excluding cancelled navigations) are captured for incident triage; expected Next deploy-skew hard reload recovery is emitted as `telemetry.route_change.outdated_deployment` so it remains raw evidence without creating grouped operator incidents.
...
- Browser session-health evidence is captured through `/api/log/browser-session` into `browser_crash_sessions`; Chrome Reporting API crash reports are advertised through `Reporting-Endpoints` and accepted at `/api/browser-crash-report` only when they correlate to an existing browser session row. `/admin/crashes` reviews account-linked freezes, stale heartbeats, abandoned previous sessions, pressure/stall snapshots, browser storage-estimate snapshots, and browser-delivered crash reports without mixing them into grouped app incidents. Crash rows preserve high-water pressure evidence across later heartbeat/lifecycle updates, and keep separate evidence status and operator review status so reviewed or ignored rows can leave the active queue while remaining searchable in History and All Evidence.
- Browser peer detection uses bounded local v2 records keyed by tab and page-session identity. Surviving documents scan every 15 seconds, report visible peers after 65 seconds without an update, and give hidden peers a 10-minute grace period for timer throttling. A record remains retryable until the server acknowledges abandonment, then it is pruned; unacknowledged records expire after 24 hours. During the one-release migration window, the client reads the legacy `shortpulse.browser_session.last.v1` / `active_tabs.v1` markers once and deletes them. Remove the legacy reader/constants after one full deployed release cycle with v2 active, once rollback to a pre-v2 browser bundle is no longer supported; do not retain both formats as permanent authorities.
...
2. Normalization + storage:
   ...
   - Browser crash-session rows stored in `browser_crash_sessions` with service-role-only table access; the authenticated browser-session API accepts only allowlisted lifecycle metadata and trusted server request headers for browser/host attribution. Chrome native crash delivery uses the `default` endpoint in `Reporting-Endpoints`; the public Reporting API crash endpoint is body-limited, rate-limited, and may update only an existing row identified by `CrashReportContext`. It does not create anonymous rows, and authenticated lifecycle clients cannot self-assert confirmed crashes. The service-role-only `record_browser_crash_session_event_v1` RPC serializes authenticated lifecycle, abandonment, and native-report mutations per browser session, uses database receipt time for ordering/staleness, and atomically merges bounded incoming evidence. Typed high-water columns preserve absolute JS heap, used-to-limit, sustained-memory, visible-stall, and peer-abandonment evidence. Latest bounded Reference Grid, Canvas, and duration-probe counters ride routine session events and selected `CrashReportContext` fields so terminal renderer loss does not erase all causal media evidence. The service-role-only `list_browser_crash_sessions_v2` RPC is the shared Admin/Badearsai authority for effective classification, reason, filtering, count, and pagination. Browser storage-estimate fields are diagnostic-only origin estimates from `navigator.storage.estimate()` when the browser supports it; they are not customer media-storage quota authority.
...
4. Operator retrieval:
   ...
   - `/api/admin/errors` = grouped incidents for the Codex handoff queue, excluding provider-sensitive safety-success rows and expected retry/rate-limit outcomes by default; `view=history` returns the retained grouped incident ledger including hidden queue rows
   - `/api/admin/crashes` and `/admin/crashes` = browser session-health rows with derived stale-session status for likely freezes or ungraceful exits; the default Needs Review queue includes absolute-memory probable rows and browser-delivered confirmed crashes, while recovered stall evidence alone remains forensic context and low-confidence stale active rows remain searchable through Possible exit and All Evidence filters; `/api/admin/crashes-status` updates open/resolved/ignored review state and optional review notes
...
- `browser_crash_sessions` rows are crash-session evidence, not grouped incidents. Browser process death cannot reliably send a final authenticated event, so classify likely freezes by stale authenticated heartbeats and `previous_session_abandoned` reports on the next authenticated page load; when Chrome can deliver a Reporting API `crash` report, `/api/browser-crash-report` promotes the existing row to `confirmed_crash`. Needs Review should stay focused on probable or confirmed crash rows; low-confidence stale active rows and abandoned exits without severe freeze evidence remain available in Possible exit and All Evidence so the table preserves forensic breadcrumbs without turning routine stale sessions into default operator work. Crash Logs presents recorded reasons when available and labels JS heap bytes and percent-of-limit separately from the legacy used-to-allocated ratio. An empty Needs Review result means only that no open probable or confirmed rows matched that filter; it is not proof that All Evidence is empty or that no user experienced a crash. Do not manually delete rows during review; use `review_status` (`open` -> `resolved`/`ignored`, with `open` for reopen) plus `review_note` to clear handled rows from Needs Review while preserving searchable forensic history.
- Use incident status transitions (`open` -> `resolved`/`ignored`, with `reopen` when needed) to represent triage state.
- Admin UI focuses on copying grouped incident triage packets into Codex and resolving completed rows. Copying a row, or using `Copy all` to copy every row in the current queue view, marks open rows in progress locally; each listed row has `Resolve` and `Resolve + Watch` actions that update `/api/admin/errors-status` and clear it from the default open queue. `Resolve + Watch` stores watch metadata and a note in incident status history for later History review. Bulk status transitions remain available through `/api/admin/errors-status-bulk` for API/tooling use.
- Provider-sensitive/refusal generation messages and expected retry/rate-limit outcomes are retained in incident storage but excluded from the default `/admin/errors` queue; use the History view when full grouped incident history is intentionally needed, and treat those rows as successful safety or admission handling unless a separate user-impacting failure signature appears.
...
- Use `Copy triage` or `Copy all` in the Incidents table for grouped-incident handoff packets, and `Copy triage` in the Event Stream table for single-event handoff packets. `Copy all` includes every row in the current queue view, including locally in-progress rows. These payloads are versioned and intentionally compact (key identifiers + normalized triage metadata) to keep troubleshooting reproducible without pasting full raw metadata blobs.
