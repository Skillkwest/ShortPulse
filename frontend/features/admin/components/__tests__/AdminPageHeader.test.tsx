...
describe("AdminPageHeader", () => {
  it("renders Customer Support first in the admin nav", () => {
    ...
    expect(links[7]).toHaveTextContent("Agent Tester Reports");
    expect(links[7]).toHaveAttribute("href", "/admin/tester-reports");
    expect(links[8]).toHaveTextContent("Errors");
    expect(links[8]).toHaveAttribute("href", "/admin/errors");
    expect(links[9]).toHaveTextContent("Legal");
    expect(links[9]).toHaveAttribute("href", "/admin/legal");
    expect(links).toHaveLength(10);
    expect(within(nav).queryByRole("link", { name: "Storage" })).not.toBeInTheDocument();
