-- Add project-owned Media Library custom folders and membership tables.
-- `All Media` remains global; custom folders become project-scoped organization only.

DO $$
BEGIN
  IF to_regclass('public.projects') IS NULL THEN
    RAISE EXCEPTION 'public.projects table is required before applying migration 094';
  END IF;
  IF to_regclass('public.media_files') IS NULL THEN
    RAISE EXCEPTION 'public.media_files table is required before applying migration 094';
  END IF;
  IF to_regclass('public.media_prompts') IS NULL THEN
    RAISE EXCEPTION 'public.media_prompts table is required before applying migration 094';
  END IF;
END;
$$;
