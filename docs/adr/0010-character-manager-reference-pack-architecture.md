# ADR 0010: Character Manager Reference-Pack Architecture

## Status
Accepted

## Context
Character consistency is moving to a structured reference-sheet workflow instead of LoRA training.  
The product needs a beginner-friendly Character Manager that:
- Captures a fixed set of required reference shots.
- Produces deterministic, versioned reference packs for generation.
...
- Use a fixed **10-slot required reference sheet** for v1.
...
## Consequences
- Positive:
  - Deterministic and reproducible character reference packs with version history.
