...
const KIE_TEMP_CHARACTER_IMAGE_URL =
  "https://tempfile.aiquickdraw.com/shortpulse/kie-video/images/character.png";
...
const mockFalUploadRouteForInputs = ({
  fileUrls = {},
  storagePaths = {},
}: {
  ...
  vi.mocked(fetchWithAuth).mockImplementation(async (_url, init) => {
    const payload =
      typeof init?.body === "string"
        ...
        : {};
    const mappedUrl = payload.storagePath
      ? storagePaths[`${payload.mediaKind ?? ""}:${payload.storagePath}`]
      : payload.fileUrl
        ? fileUrls[`${payload.mediaKind ?? ""}:${payload.fileUrl}`]
        : undefined;
...
const makeArgs = (overrides: Partial<VideoSubmissionArgs> = {}): VideoSubmissionArgs => ({
  ...
  rawImageInputs: ["https://example.com/character.png"],
...
describe("handleVideoModelSubmission (Lip Sync)", () => {
  ...
  it("submits clean image/audio payload with optional prompt and turbo mode", async () => {
    ...
    const stagedImageUrl = "https://v3.fal.media/files/lip-restaged.png";
    const stagedAudioUrl = "https://v3.fal.media/files/voice-restaged.mp3";
    mockFalUploadRouteForInputs({
      fileUrls: {
        [`image:${imageUrl}`]: stagedImageUrl,
        [`audio:${audioUrl}`]: stagedAudioUrl,
      },
    });
    const args = makeArgs({
  ...
  it("submits ready uploaded Lip Sync audio without reading the local preview URL", async () => {
    ...
    mockFalUploadRouteForInputs({
      ...
      storagePaths: {
        "audio:user-1/audio/reference-grid/local-voice.mp3": uploadedAudioUrl,
      },
    });
    const args = makeArgs({
      finalModel: FAL_OMNIHUMAN_V15_MODEL_ID,
  ...
  it("uploads Lip Sync character storage refs to Fal CDN before provider submit", async () => {
    ...
    mockFalUploadRouteForInputs({
      ...
      storagePaths: {
        "image:user-1/images/character.png": falImageUrl,
      },
    });
    const args = makeArgs({
      finalModel: FAL_OMNIHUMAN_V15_MODEL_ID,
  ...
  it("uses Lip Sync prepared Supabase signed image storage path for Fal CDN staging", async () => {
    ...
    mockFalUploadRouteForInputs({
      ...
      storagePaths: {
        "image:user-1/reference-images/prepared-character.png": falImageUrl,
      },
    });
    const args = makeArgs({
      finalModel: FAL_OMNIHUMAN_V15_MODEL_ID,
  ...
  it("uses the prepared Lip Sync image URL instead of rereading local image previews", async () => {
    ...
    mockFalUploadRouteForInputs({
      fileUrls: {
        [`image:${preparedImageUrl}`]: falImageUrl,
        [`audio:${audioUrl}`]: stagedAudioUrl,
      },
    });
    const args = makeArgs({
  ...
  it("uploads remote Lip Sync audio URLs through the URL upload path", async () => {
    ...
    mockFalUploadRouteForInputs({
      fileUrls: {
        [`image:${imageUrl}`]: stagedImageUrl,
        [`audio:${remoteAudioUrl}`]: uploadedAudioUrl,
      },
    });
    const args = makeArgs({
      finalModel: FAL_OMNIHUMAN_V15_MODEL_ID,
  ...
  it("fails the Lip Sync output when Fal input staging hangs before provider submit", async () => {
    vi.useFakeTimers();
    const imageUrl = "https://v3.fal.media/files/lip.png";
    const audioUrl = "https://v3.fal.media/files/voice.mp3";
    ...
    const pending = handleVideoModelSubmission(args);
    ...
    expect(args.notifyGenerationFailure).toHaveBeenCalledWith(
      "out-1",
      "Character image preparation failed: Lip Sync media staging timed out before provider submit.",
      undefined,
      {
        reasonCode: "USER_INPUT_VALIDATION",
        telemetryMode: "validation",
...
describe("handleVideoModelSubmission (Kling 3 motion)", () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(fetchWithAuth).mockImplementation(async (_url, init) => {
      const body =
        typeof init?.body === "string" ? (JSON.parse(init.body) as { uploadPath?: string }) : {};
      const url =
        body.uploadPath === "shortpulse/kie-video/images"
          ? KIE_TEMP_CHARACTER_IMAGE_URL
          : KIE_TEMP_MOTION_REFERENCE_URL;
  ...
  it("builds and submits a motion payload with normalized prompt and mode", async () => {
    ...
    expect(submitKieKlingImageToVideo).toHaveBeenCalledWith({
      ...
      image_urls: [KIE_TEMP_CHARACTER_IMAGE_URL],
  ...
  it("refreshes signed Kling element media before submit", async () => {
    ...
    vi.mocked(fetchWithAuth)
      ...
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({
          url: "https://tempfile.aiquickdraw.com/shortpulse/kie-video/images/start.png",
        }),
      } as Response);
  ...
  it("uploads Kling element reference images to Kie temporary storage before submit", async () => {
    ...
    expect(submitKieKlingImageToVideo).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        image_urls: ["https://tempfile.aiquickdraw.com/shortpulse/kie-video/images/start.png"],
        kling_elements: [
          {
            name: "element1",
            description: "Reference images for Taylor",
