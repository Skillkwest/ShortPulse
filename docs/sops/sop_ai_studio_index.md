...
- `Sound -> Voice` keeps voice management inside the dedicated Voices panel: the main header owns the `Voices` modal trigger, while the modal header owns `Create New Voice` and selected-voice deletion. Voice hydration still comes from `GET /api/elevenlabs/voices`, and deletion still uses `DELETE /api/elevenlabs/voices/:voiceId` for saved/provider cleanup. The selectable voice chip library lives inside that modal instead of the main panel body.
