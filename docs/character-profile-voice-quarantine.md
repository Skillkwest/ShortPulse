...
- The visible voice selector and `+ Create Voice` CTA are removed from `frontend/features/character-manager/components/CharacterManagerShell.tsx`.
...
3. Re-introduce the UI in `CharacterManagerShell` only after the flow is end-to-end functional.
