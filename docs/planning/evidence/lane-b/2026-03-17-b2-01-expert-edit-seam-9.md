# Lane B Evidence Packet: B2-01 Expert Edit Seam 9
...
## Scope
1. Extract stage context-menu remove-image layer reset transition from `ExpertEditPanelView` into `expertEditLayerSessionUtils`.
2. Keep context-menu behavior and layer reset contract unchanged.
...
4. `docs/planning/evidence/lane-b/2026-03-17-b2-01-expert-edit-seam-9.md`
...
1. `ExpertEditPanelView.tsx`: `5287` -> `5278` (`-9` lines, size-budget script count).
2. `expertEditLayerSessionUtils.ts`: expanded with `resolveLayersAfterContextMenuRemoveImage` utility.
3. Coupling reduction:
   - Stage context-menu remove-image layer-reset mapping now lives in layer-session utility module.

## Parity Assertions
1. `ExpertEditPanelView` prop contract remained unchanged.
2. No API/server/schema changes.
3. Stage context-menu remove-image behavior remained unchanged; extraction-only seam.
...
## Rollback Note
1. Revert this slice commit to inline stage-menu remove-image layer reset mapping back into `ExpertEditPanelView`.
