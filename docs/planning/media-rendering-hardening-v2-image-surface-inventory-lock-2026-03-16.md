...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| IMG-HOT-001 | media-library-route | `frontend/features/media-library/components/MediaAssetGallery.tsx` | `img` | hot-path | Engineering | signed storage, trusted direct | signed URLs currently bypass adaptive transform path | surface-derived preview profile | media library grid tests | Route gallery hot path |
...
| create-beginner-character-trigger | `frontend/features/ai-studio/components/create/BeginnerCreatePanelView.tsx` | `next/image` | `ui-chrome-or-static` | Character picker trigger avatar plus model logo; user-visible selection chrome, not a media-library delivery surface |
