...
| Local agent credentials  | Local `.env*.local` and `.env.agent.local` files may support supervised pre-launch agent work, but must remain ignored and must not be copied into Git-tracked docs, reports, storage-state files, or temporary exports. |
