...
- Expert Edit panel contracts:
  ...
  16. Expert Edit prompt-reference tokens support `@img1`, `@img2`, and `@img3` for standard edit secondary slot references, with generate-time validation blocking and submit-time Figure mapping for provider prompts. The current FLUX Fill inpaint lane is `@main`-only and does not transmit secondary reference images.
