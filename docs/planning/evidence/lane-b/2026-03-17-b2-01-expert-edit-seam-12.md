# Lane B Evidence Packet: B2-01 Expert Edit Seam 12
...
## Scope
1. Extract transient object-URL revoke timer scheduling/cleanup logic from `ExpertEditPanelView` into `expertEditInteractionUtils`.
2. Keep object-URL revoke behavior and unmount cleanup behavior unchanged.
...
4. `docs/planning/evidence/lane-b/2026-03-17-b2-01-expert-edit-seam-12.md`
...
1. `ExpertEditPanelView.tsx`: `5247` -> `5245` (`-2` lines, size-budget script count).
2. `expertEditInteractionUtils.ts`: `123` -> `160` (`+37` lines) with reusable transient object-URL revoke timer helpers.
3. Coupling reduction:
   - Timer scheduling/overwrite/clear logic for transient object URLs moved to interaction utility.
   - Unmount cleanup path now uses one shared utility for transient timer drain + revoke behavior.

## Parity Assertions
1. `ExpertEditPanelView` prop contract remained unchanged.
2. No API/server/schema changes.
3. Transient object-URL revoke timing and cleanup behavior remained unchanged; extraction-only seam.
...
## Rollback Note
1. Revert this slice commit to inline transient object-URL revoke timer scheduling/cleanup logic back into `ExpertEditPanelView`.
