/**
 * Proxies Fal.ai Seedance 1.5 Pro text-to-video submit requests.
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const upstream = await fetch(FAL_SEEDANCE_SUBMIT_URL, {
      method: "POST",
      headers: {
    ...
    return res.status(500).json({ error: "Fal Seedance submit failed", detail: String(error) });
  } finally {
