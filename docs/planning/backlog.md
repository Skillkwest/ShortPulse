...
- Hybervees highest-ROI backlog: make generated-media context recovery obvious from the selected Media Library or Reference Grid item path. A saved generated asset should show the original prompt/setup when available and a clear reload/reuse action, or explain why that context is unavailable. Start with current-production validation of generated media detail/reload behavior; do not duplicate `Prompts` filter semantics if prompt cards are intentionally separate. Source: `docs/records/artifacts/agent/hybervees/reports/2026-07-07-maya-find-generated-image-context-insight-review.md` / Maya found her paid image but would not spend credits on a variant because the prompt/setup was not obvious.
