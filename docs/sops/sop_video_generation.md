...
- Before Fal OmniHuman submit, the Lip Sync adapter stages the selected character image and voice audio through `/api/fal/upload-url` so upstream `image_url` and `audio_url` values are freshly verified Fal CDN URLs. App-owned media must stage from caller-owned storage paths when storage authority is known; signed/display URLs and existing Fal CDN URLs are re-staged as URL-only media with explicit `mediaKind` so OmniHuman receives image/audio MIME-compatible files. Client-side staging requests and server-side Fal CDN initiate/upload calls must be timeout-bounded; a staging timeout fails the optimistic output before provider submit rather than leaving a spinning placeholder. The server submit proxy rejects OmniHuman requests before billing/provider dispatch when either final URL is not on the Fal CDN. Do not route Lip Sync source intake through `/api/upload-audio`, and do not route Lip Sync provider staging through `/api/kie/upload-url`.
...
- Motion Control remains separate and uses the Kie motion-control route (`model = kling-3.0/motion-control`) with one character image, one provider-facing MP4/MOV motion video, `mode=720p|1080p`, and the Motion audio setting. Motion does not submit standard-video `aspect_ratio` or `duration`; text direction is optional once both motion inputs are present. Local character-image files selected in the Motion slot stage through the canonical reference-image upload contract first, so oversized stills use the shared image admission/normalization path before the signed character image URL is committed.
