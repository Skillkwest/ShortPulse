...
Purpose: give Beeper an owned workspace folder inside the repo for professional alpha-testing checklists, scratch planning, and lightweight tooling that should stay separate from product code.
...
- Beeper should keep thorough workflow/UI/UX reports inside `docs/agents/beeper/workspace/reports/` so the long-form analysis stays in Beeper's owned folder.
- Beeper should keep reusable product-learning syntheses inside `docs/agents/beeper/workspace/findings/` so future work can load the highest-signal truths before reopening old run packets.
- Beeper should keep short user-facing checkpoint summaries inside `docs/agents/beeper/workspace/checkpoint-summaries/` so the user can scan each checkpoint quickly and correct Beeper during training.
...
- Beeper should keep a durable route/control/action history inside `docs/agents/beeper/workspace/action-coverage/` so each new run can target different product actions over time.
- Beeper should keep one defined normal-user success target per major route in `docs/agents/beeper/workspace/route-success-map.md` so coverage claims are tied to believable outcomes instead of just clicks.
- Beeper should use `docs/agents/beeper/workspace/next-run-queue.md` to keep the highest-value next testing lanes visible and reduce lane-selection churn.
...
- Beeper should keep raw screenshots, network captures, storage-state dumps, and other sensitive packets only in the ignored local cache at `docs/agents/beeper/workspace/evidence-cache/`.
- Retained docs under `docs/records/artifacts/agent/beeper/` remain the compact durable ledger, while `docs/agents/beeper/workspace/reports/` can hold the denser working audit write-up.
...
- Average-user comparison work belongs to the standalone Bopper lane, not Beeper's active workspace.
- Beeper's workspace should stay segregated from Bopper's by default. Do not copy Bopper notes, summaries, or confusion artifacts into `docs/agents/beeper/workspace/` unless the user explicitly asks for a comparative synthesis.
...
- Create a kept detailed checkpoint report during testing:
  `node docs/agents/beeper/workspace/scripts/start-checkpoint-report.mjs --slug <name> --environment production --workflow <lane>`
