...
| Create panel UI               | `frontend/features/ai-studio/components/CreatePropertiesPanel.tsx` + `components/create/StandardCreatePropertiesPanel.tsx` + `components/create/PulseCreatePropertiesPanel.tsx`                  | Performs the top-level Standard/Pulse UI switch, then enters mode-owned composer modules.                                                          |
...
| Agent client bridge           | `frontend/features/ai-studio/hooks/useAiStudioAgentBridge.ts` + `useAiStudioAgentOrchestration.ts` + `../../ai-agent/useStandardCreateAgent.ts` + `../../ai-agent/usePulseCreateAgent.ts`                                                             | Owns lane-scoped chat state, attachment prep, prompt application, and agent-output-to-generate handoff.                                                        |
| Agent runtime + control plane | `frontend/pages/api/ai/studio-agent-standard.ts`, `frontend/pages/api/ai/studio-agent-pulse.ts`, Standard runtime `frontend/features/agent-runtime/standardStudioAgentRuntime/runtime.ts`, Pulse runtime `frontend/features/agent-runtime/pulseStudioAgentRuntime/runtime.ts`, retired compatibility route `frontend/pages/api/ai/studio-agent.ts`, `frontend/pages/api/ai/extract-style.ts`, `frontend/lib/server/api/agentSafetyPolicyControlPlane.ts`, `frontend/pages/api/admin/agent-safety-policy/*` | Server-authoritative agent execution, lane boundary validation, safety precheck/profile resolution, style extraction intake, and admin activation/rollback/version controls. |
...
3. `CreatePropertiesPanel` performs the top-level Create-mode render decision:
...
8. In Expert Create `Pulse` mode, `PulseCreatePanelView` mounts `CreateExpertPresetPanel` in the left rail. Custom Pulse preferences are mounted through the Pulse-only `CreatePulsePreferenceProvider`, so Standard mode does not load saved custom Pulse definitions into active page state. That panel uses the shared per-user Pulse catalog and curated rail allocation from `user_preferences`, supports drag/drop from `CreatePulsePresetsSurface`, allows built-in Pulse presets to be edited as per-user overrides on their seeded ids, and activates a pinned preset by setting the current Pulse runtime selection instead of mutating the visible `agentInput` composer. `CreatePulsePresetsSurface` now serves both as a browse/pin surface and as a direct activation surface: clicking a Pulse there activates it immediately and pins it into the rail so the active owner remains visible. The library panel (`Libraries -> Presets -> Pulses`) remains catalog-management only and does not activate Pulses directly. Pulse definitions carry the current guided GPT-style contract: `workflow_gpt`, `activate_and_start`, and `chat_reply`. The built-in workflow starter set includes exactly `Video Prompt Magic`, `Multi Sequence Video Prompt`, and `DFY Story Builder`; custom Pulse authoring uses only `Preset Name` and `System Instructions` while targeting the same guided behavior. Retired Pulse metadata is discarded during preference normalization. Pulse is an agent-only lane: it does not read or own the Standard Create chat toggle, it uses the Pulse agent hook and `/api/ai/studio-agent-pulse`, it hides the inline chat-mode toggle, and it suppresses the shared `StylesControl` plus right-rail Styles toggle until the user switches back to `Standard`. Switching back to `Standard` clears active Pulse runtime ownership; returning to `Pulse` starts with no active Pulse until the user starts one.
9. The right-side Agent Chat rail is retired. Create agent chat remains inside the active Create composer only; `AiStudioShellFrame` should not render a generic `AgentChatPanel` or accept a mixed `agentChat` contract.
10. Pulse runtime state is page/session-owned and threads into `/api/ai/studio-agent-pulse` as hidden runtime metadata. Standard agent traffic routes through `/api/ai/studio-agent-standard`, which rejects Pulse-shaped payloads. The workspace snapshot persists lane-specific `agentRuntimes` only; the legacy generic `agent` field is not a conversational restore authority. Standard-mode restore must fail closed against hidden Pulse runtime state: it must not hydrate active Pulse ownership, `pulseWorkflowSession`, transcript state, draft agent input, `promptOrigin`, or the Standard Create chat-toggle preference. On project routes, persistence is narrowed further by ADR 0070: project workspace restore may preserve the shell mode, but it must not hydrate active Pulse runtime ownership or conversational runtime state.
11. Active Pulse sessions use the authoritative `pulseWorkflowSession` runtime state for guided status, step progression, and artifact completion, but the Create chat surface no longer renders separate Pulse session banner chrome. The visible workflow guidance should come from the assistant turns themselves, with the transcript remaining the primary surface the user reads during a Pulse. Project reload must not be treated as workflow-session restore; on project routes guided Pulse context should reappear only after a fresh Pulse activation regenerates that conversational runtime.
...
- Chat mode ON: send to the Standard chat lane. When `NEXT_PUBLIC_STUDIO_AGENT_DIRECT_OPENAI_BYPASS_ENABLED=true`, the Create/Text chat surface defaults to `directOpenAiBypass=true` so `/api/ai/studio-agent-standard` talks directly to OpenAI without a separate UI toggle.
- Chat mode OFF: resolve raw prompt via `resolveChatOffCreatePrompt`, then submit generate as Create/Image.
- Expert Create `Pulse` mode override: Pulse is its own agent-only lane and does not consult the persisted Standard Create chat-toggle preference; switching back to `Standard` restores the prior Standard Create chat-toggle preference.
...
- `useStandardCreateAgent` and `usePulseCreateAgent` bind the reusable agent state engine to explicit Standard/Pulse transports and response parsers. Standard posts only to `/api/ai/studio-agent-standard`; Pulse posts only to `/api/ai/studio-agent-pulse`.

2. Studio-agent server path:

- `/api/ai/studio-agent-standard` and `/api/ai/studio-agent-pulse` enforce lane payload boundaries before invoking their mode-owned runtime modules. The generic `/api/ai/studio-agent` route is retired and returns `410` for authenticated POST requests.
...
- `/api/ai/studio-agent-standard` accepts `directOpenAiBypass=true` in the request envelope.
...
- Pulse preset click/apply in Expert Create activates hidden Pulse runtime behavior on `/api/ai/studio-agent-pulse` instead of writing preset text into the visible Create composer. The Pulse Presets Library and Create Pulse rail share the same persisted Pulse catalog, including per-user overrides for built-in starter Pulses, and the active Pulse flows through the Pulse agent request contract.
...
- Manual smoke in `/ai-studio`:
  ...
  4. In Create chat mode with the bypass flag enabled, Standard agent sends bypass orchestration and hits the direct OpenAI branch inside `/api/ai/studio-agent-standard`.
  ...
  8. Switching `Pulse -> Standard -> Pulse` clears the prior hidden Pulse transcript/workflow runtime and does not show Pulse messages in Standard mode.
