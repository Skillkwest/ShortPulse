...
- `Sound -> Voice -> Create New Voice` now has two modal creation methods. `Generate Voice` preserves the existing Voice Design preview/select/save flow through `POST /api/elevenlabs/text-to-voice/design` and `POST /api/elevenlabs/text-to-voice/create`. `Clone Voice` is audio-only in v1: local or Reference Grid audio samples stage through `POST /api/media/stage-voice-clone-source` under the caller's private `voice-clone/source-audio` namespace, require inline permission confirmation, then submit the staged `sourceStoragePath` to `POST /api/elevenlabs/voices/clone` to create and persist a reusable provider voice. Clone sources must be at least 1 minute long. Clone mode intentionally does not reuse Voice Changer storage namespaces or `speech-to-speech` generation routes.
