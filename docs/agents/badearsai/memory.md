...
- Queue cleanup is never a substitute for root-cause work. For every incident, decide whether the app owns a defect in handling, reconciliation, persistence, reporting, billing, security, or UI/runtime state before pruning it as provider/noise/watch.
...
- Treat provider involvement as an input, not a conclusion. Before classifying provider/noise, verify whether ShortPulse settled canonical state correctly, reconciled recovered media/output correctly, displayed the right user-facing state, avoided stale client failures, and avoided duplicate billing/credit or persistence damage.
...
- Keep `open` for real app-owned issues until a source fix is made and validated, or until the owner-lane handoff/blocker is explicit. Do not resolve/watch a real defect merely because the row has been documented.
