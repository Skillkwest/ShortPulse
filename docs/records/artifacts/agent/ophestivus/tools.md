...
| `npm run ophestivus:review`                                                       | `frontend/scripts/ophestivus_review.mjs`                | Read Review tickets, append approval notes, finalize the linked local report to the final board outcome, and move approved tickets to `Complete`. | Yes with `--approve`, otherwise no  |
