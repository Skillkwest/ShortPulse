...
| `frontend/pages/api/ai/studio-agent-standard.ts` / `frontend/pages/api/ai/studio-agent-pulse.ts`                                                                                                                                                                                      | Mode-owned API routes. Standard is a raw OpenAI pass-through for user/assistant turns with lane-boundary validation only. Pulse keeps the guided workflow, request guards, and structured runtime behavior. |
...
- Standard lane contract: no local system instructions are injected. User/assistant turns are forwarded to OpenAI as-is, with attached image media passed through as multimodal user content when present.
...
5. Standard provider execution path:
   - forwards raw user/assistant history to OpenAI with no local Standard system prompt, no Standard client/server precheck, and no Standard canonical-prompt loop.
