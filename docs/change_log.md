...
- Documented Fal.ai model workflows (`docs/api-fal-kling-video.md`, `docs/api-fal-kling-2-5-image-to-video.md`, `docs/api-fal-veo3.md`, `docs/api-fal-flux-dev.md`, `docs/api-fal-nano-banana-pro.md`, `docs/api-fal-seedream-4-5.md`, `docs/api-fal-seedance-1-5-pro.md`) so every queue/task/callback path is captured plus the backend `kei/task-status` proxy.
...
- Added Kling 2.5 Turbo Pro (Fal) image-to-video queue with per-duration pricing and default 10s runs; enforced per-model duration defaults in cost estimates/debits.
