...
- Media preview signing: `/api/media/sign-batch` signs user-scoped media preview paths in a single authenticated request with surface-aware preview profiles (route/modal/panel). Signed transforms are policy-gated and disabled by default unless both `SHORTPULSE_MEDIA_SIGNED_TRANSFORMS_ENABLED=true` and `NEXT_PUBLIC_MEDIA_SIGNED_TRANSFORMS_ENABLED=true`.
