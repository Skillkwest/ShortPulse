...
import { useCreateAgentOrchestrationRuntime } from "./useCreateAgentOrchestrationRuntime";
...
type UsePulseCreateAgentSendParams = Pick<
...
/**
 * Pulse Create guided send path. Owns workflow request shaping, workflow
 * session capture, and Pulse-specific input guards.
 */
export const usePulseCreateAgentSend = ({
  agentIsSending,
  agentBootstrapReady,
  agentUiBusyRef,
  setAgentUiBusy,
...
}: UsePulseCreateAgentSendParams) => {
  const pulseSendRuntime = useCreateAgentOrchestrationRuntime({
    runtimePolicy,
    setPulseWorkflowSession,
    setUiNotice,
    setAgentAttachmentError,
  ...
  return useCallback(
    async (
      ...
      if (!pulseSendRuntime.ensureSessionReady()) {
        return;
      }
      ...
      const selectedOverride = pulseSendRuntime.resolveSelectedOverride(options?.selectedOverride);
      const baseContext = getAgentContext({
        lastAssistantMessage,
        selectedOverride,
        includeActiveOutput: pulseSendRuntime.includeActiveOutput,
        modeHint: options?.modeHint ?? (outboundAttachments.length ? "reference" : undefined),
      });
      if (!pulseSendRuntime.hasRequiredRequestContext(baseContext)) {
        return;
      }
      if (
        await pulseSendRuntime.blockInvalidUserInput({
          context: baseContext,
          hasImageAttachment,
        })
      ) {
      ...
      try {
        ...
        const requestContext = await pulseSendRuntime.prepareUserInputRequestContext({
        ...
        const hasActivePulse = pulseSendRuntime.hasPromptApplyPulseContext(requestContext);
        if (appliedPrompt) {
        ...
        await pulseSendRuntime.captureWorkflowSession(workflowSession);
