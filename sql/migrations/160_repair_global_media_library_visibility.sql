-- Repair global Media Library visibility for hosted production paths.
-- This is the temp-table-free version of the All Media completeness backfill so it
-- can run through transaction/session poolers that do not preserve temp tables.
...
with variant_paths as (
    ...
    select
        mf.user_id,
        trim(to_jsonb(mf)->>'thumb_variant_path') as storage_path
    ...
    select
        mf.user_id,
        trim(to_jsonb(mf)->>'poster_variant_path') as storage_path
    from public.media_files mf
    ...
    select
        mf.user_id,
        trim(to_jsonb(mf)->>'preview_variant_path') as storage_path
    from public.media_files mf
    ...
    select
        ...
        case
            ...
            when so.storage_path like so.user_id::text || '/uploads/audio/%' then 'uploads_audio'
            when so.storage_path like so.user_id::text || '/audio/%' then 'legacy_audio'
            ...
            when so.storage_path like so.user_id::text || '/generations/audio/%' then 'generations_audio'
            else null
...
select
    ...
    jsonb_strip_nulls(
        jsonb_build_object(
            'backfill_migration', '160_repair_global_media_library_visibility',
