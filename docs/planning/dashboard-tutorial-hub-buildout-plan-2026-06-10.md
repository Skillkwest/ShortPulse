...
- Add a private app-owned dashboard tutorial thumbnail bucket plus signed-upload prepare/finalize routes so admins can drop local GIF/image/video thumbnails without manually hosting them. Uploaded originals are source material; dashboard runtime cards should use small app-owned display derivatives generated by finalization/backfill.
