...
Purpose: retain Datserok's small helper inventory for project persistence work without becoming another default-load surface.
...
- Create new templates or scripts only after two or more real Datserok runs repeat the same manual checklist or evidence shape.
- Until then, prefer the source map, runtime load policy, and reports index over adding more helper documents.
