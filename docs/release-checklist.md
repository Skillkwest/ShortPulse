...
- Auth public signup gate: for account-first signup launch, `sql/migrations/164_add_paid_signup_intent_gate.sql` and `sql/migrations/165_account_first_signup_intent_gate.sql` are applied, Supabase Auth's Before User Created hook calls `public.hook_shortpulse_signup_intent(event jsonb)`, Supabase Auth public signup is enabled only after hook proof, and `NEXT_PUBLIC_SHORTPULSE_PUBLIC_SIGNUP_ENABLED` is unset or set to `true` in production. Set it to `false` only for an emergency app-level signup close.
