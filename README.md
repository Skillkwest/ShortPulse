...
- AI Studio style extraction helper route: authenticated `POST /api/ai/extract-style` extracts reusable style descriptors from uploaded/dropped images used in Styles Library create flows.
...
- AI Studio Media Library folder APIs: `/api/media/folders/list|create|rename|delete|membership-batch` plus `/api/media/prompts/list` provide user-scoped folder CRUD, membership assignment/unassignment, and prompt pagination for the left-panel Media Library surface.
...
- **AI Studio (`/ai-studio`)**: Creative canvas for prompt systems, model/aspect selection, previewing, and saving image/video outputs. Session identity uses `?sid=<uuid>`; plain `/ai-studio` creates a new session id. Beginner mode is temporarily force-disabled and toggle controls are hidden by runtime policy flags. Expert Edit properties panel is the default Edit experience; set `NEXT_PUBLIC_ENABLE_EXPERT_EDIT_UI=false` to force legacy Edit panel fallback.
  ...
  `Libraries -> Media Library` opens a primary left-panel Media Library surface (tool id `media-library`) with root/custom folder organization and media/prompt browse/add flows; set `NEXT_PUBLIC_AI_STUDIO_MEDIA_LIBRARY_PANEL_ENABLED=false` to fallback to the legacy modal path during staged rollback.
  `Shortcuts -> Styles` opens the primary left-panel Styles Library (shared selection with Expert Create/Edit) and keeps the right rail in reference-grid-only mode; the Styles Library supports drag/drop reordering of style cards in-grid, drag/drop image intake from your computer/Reference Grid/Quick Slot to auto-create custom styles, and an `Add Style` tile that opens a create-style modal. Clicking a style card opens an edit modal for `Style`, `Reference Image`, and `Style Prompt`; dropped/uploaded reference images are center-cropped to a square for the style preview card. `Shortcuts -> Presets` opens the primary left-panel Presets Library using the Expert Edit preset catalog with `Custom 1`, `Custom 2`, and `Custom 3` placeholder tiles plus a full-size `Create New Preset` tile. Clicking a preset card opens an edit modal for preset name + prompt text. Each preset card includes a top-right delete button that opens a yes/no confirmation modal before permanently deleting that preset from the user library. Clicking `Create New Preset` opens the modal in create mode for the next custom slot (`Custom 4+`). Saves persist via the shared per-user preset override preference.
  New-style creation from image intake now auto-runs style extraction to populate `Style Prompt` with reusable descriptors (GPT-5-nano vision path), while keeping manual prompt editing available.
