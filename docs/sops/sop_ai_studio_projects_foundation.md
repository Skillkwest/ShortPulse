...
8. Project workspace persistence now keeps only durable project content: active output media for the shared right rail, Quick Slot Inventory and Reference Grid projection ids, durable Canvas scene items and cameras, the project-owned Pulse `Chats` library, plus the separately owned project title. The Media Library folder system remains user-global and is not project-owned workspace state. Project routes do not persist workflow shell state, typed composer text, active output focus, shared reference-selection state, Character Mode shell state, automatic conversational replay, or Expert Edit document state.
9. Opening or switching a project resets the project-visible agent conversation lane instead of restoring it from project workspace state. Saved Pulse chats remain explicit reopen affordances inside that project rather than automatic visible replay on bootstrap.
...
13. Project workspace saves now normalize the compatibility snapshot into a lightweight project checkpoint plus `project_output_display_items`, then run project asset/generation association repair as a follow-up stage. Browser-built project snapshots keep durable-backed row text as bounded summaries instead of carrying full prompt, transcript, lyrics, title, or error blobs through the autosave byte gate.
14. Project workspace normalization enforces the Reference Grid visible-active cap server-side before the checkpoint/display-record sync runs. Terminal hidden rows are pruned from the hot project workspace state, hidden in-flight rows with runtime identity may remain until lifecycle settles, overflow visible rows are kept out of the hot active workspace and retained as archived cold output stubs/display records, and over-cap saves emit repair-oriented telemetry with trimmed-row samples and authority counts.
...
6. Current route contract intentionally reuses the AI Studio session snapshot envelope as a parser and transport boundary, but storage now splits durable state into a lightweight checkpoint plus `project_output_display_items`. Durable-backed output rows in the lightweight checkpoint stay compact, while `project_output_display_items` must preserve full user-visible prompt and preview text used by references, detail modals, and composer drops.
...
8. `PUT` enforces the Reference Grid visible-active cap and prunes terminal hidden rows before durable checkpoint/display work. Overflow visible rows are not retained in the hot active project workspace snapshot; restorable overflow rows are retained as archived cold output stubs and corresponding `project_output_display_items`, while operators can still use the emitted over-cap telemetry and global media/generation authority when repairing already-over-cap projects.
...
9. Project workspace reads return the sanitized compatibility snapshot materialized from checkpoint + display records, then narrowly converge existing generated rows from project-owned generation/projection/media authority. After bootstrap, AI Studio may still refresh generated-output delivery from project-owned generation rows rather than scanning all user-global generated outputs.
