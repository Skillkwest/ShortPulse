-- Add companion custom-row manifests to model pricing policy versions.
-- These rows are display-layer authoring state for /admin/pricing and do not change calculator math.
...
set custom_rows = '{"schemaVersion":1,"rowsByModel":{}}'::jsonb
where custom_rows is null or jsonb_typeof(custom_rows) <> 'object';
...
begin
    ...
    select
        active.version,
        active.policy,
        coalesce(active.custom_rows, '{"schemaVersion":1,"rowsByModel":{}}'::jsonb),
        active.id,
        safe.version,
        safe.id,
        runtime.updated_at,
