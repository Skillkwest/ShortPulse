...
- Expert Edit panel contracts:
  ...
  12. `Libraries -> Presets` opens one primary left-panel Presets Library with internal `Pulses` and `Prompt Presets` sections. The `Pulses` section merges the global built-in Pulse catalog with shared per-user custom Pulse persistence. The current seeded built-in starter set is `Video Prompt Magic`, `Multi Sequence Video Prompt`, and `DFY Story Builder`, but the active built-in set now comes from the admin control plane and may change over time. Built-ins are no longer editable from user Pulse surfaces; only custom user-authored Pulses use the library/editor flows and the guided `workflow_gpt` runtime contract. The `Prompt Presets` section is populated from the full Expert Edit preset catalog (with overrides applied), keeps the right rail visible, and supports click-to-edit preset name/prompt in a modal.
  ...
  15. Inpaint lock and reference-aware masked routing use shared helpers from `frontend/features/ai-studio/logic/inpaintSubmission.ts`. Default inpaint resolves to `INPAINT_FLUX_FILL_MODEL_ID`, while the single-reference masked lane resolves to `INPAINT_REFERENCE_MODEL_ID` when the prompt links exactly one unique secondary reference token.
  16. Expert Edit prompt-reference tokens support `@img1`, `@img2`, and `@img3` for standard edit secondary slot references, with generate-time validation blocking and submit-time Figure mapping for provider prompts. Default FLUX Fill inpaint is `@main`-only. The optional reference-aware inpaint lane allows exactly one unique `@imgN` and transmits it as `reference_image_url`.
  ...
  23. Prototype flag `NEXT_PUBLIC_AI_STUDIO_MARKUP_COLLAPSED_OPEN_MODAL_ENABLED=true` changes collapsed `Tools` click behavior to open expanded Markup modal directly and forces tools back to collapsed on modal close.
...
| Fal      | `fal-ai/nano-banana`                              | 1:1 default (allowed: 21:9, 16:9, 3:2, 4:3, 5:4, 1:1, 4:5, 3:4, 2:3, 9:16)          | Text-to-image via the Fal queue; flat per-image pricing ($0.039 -> 4 credits before per-model overrides) and PNG outputs, proxied through `/api/fal/nano-banana-*`.                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
