...
- `reference-grid` now has explicit user approval plus a retained onboarding plan, but it still needs the first production baseline before promotion beyond candidate status.
...
5. Reference Grid with approved onboarding plan and pending production baseline
