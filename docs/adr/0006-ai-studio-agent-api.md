...
- Historical note: the original Standard route kept its system prompt in code (`frontend/lib/agentPromptsConfig.ts`) under `STUDIO_AGENT_SYSTEM`. The current Standard Create contract has moved to one admin-owned runtime prompt row in the control plane; code-local prompt text remains only as non-runtime recovery/reference material.
