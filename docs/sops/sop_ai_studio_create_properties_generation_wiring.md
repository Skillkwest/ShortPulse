...
| Workflow plan access          | `frontend/lib/billing/aiStudioWorkflowEntitlements.ts`, `frontend/features/ai-studio/routes/AiStudioRouteApp.tsx`, `frontend/lib/server/api/aiStudioWorkflowPlanGuard.ts`, `frontend/lib/server/api/falSubmitProxy.ts`, `frontend/pages/api/elevenlabs/*`                                                                                                                                                                             | Blocks `free` and `starter` accounts from AI Studio Video and Sound workflows, replaces restricted rail buttons with `View plans`, and denies executable video/audio submits before billing reservation or provider dispatch. |
...
- Enforce workflow-plan access before dispatch for Video/Sound entry points. `free` and `starter` plans must see the shared `View plans` CTA instead of Video/Sound rail buttons, and the route-level server guard must reject FAL video and ElevenLabs audio generation before billing reservation or provider dispatch.
