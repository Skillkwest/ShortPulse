...
| STG-02 SQL/RPC Hardening (`028`) | In Progress | @sleepyseamonster | `frontend`, `security`, `sql_lint` | added, not yet applied in env | SQL matrix + gate script ready | `docs/planning/evidence/sql/` | added `sql/check_conversation_state_hardening_028.sql` and `scripts/conversation_state_hardening_gate.sh`; staging/prod evidence still pending |
