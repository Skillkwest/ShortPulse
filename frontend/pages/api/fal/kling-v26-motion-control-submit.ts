/**
 * Proxies Fal.ai Kling 2.6 motion control submit requests.
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const upstream = await fetch(FAL_KLING_MOTION_CONTROL_SUBMIT_URL, {
      method: "POST",
      headers: {
    ...
    return res.status(500).json({ error: "Fal Kling 2.6 motion control submit failed", detail: String(error) });
  } finally {
