...
| `2026-06-15-stripe-billing-account-security-sweep.md` | Stripe checkout, portal, webhook, subscription, storage add-on, credit, admin billing, and SQL/RLS account isolation | Reviewing current launch-readiness evidence for whether users can access, mutate, or receive another user's Stripe customer/subscription, transaction history, billing rows, or credits. |
| `2026-06-16-free-signup-billing-security-audit.md` | public signup, hidden free bootstrap tier, free credit/storage value, and zero-credit generation admission | Reviewing how production users can appear as `Free/Active`, whether free accounts received credits or provider access, and the hardening that removes paid value from hidden free metadata. |
