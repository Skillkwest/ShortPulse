# Lane B Evidence Packet: B2-01 Expert Edit Seam 14
...
## Scope
1. Extract session dispatch `requestAnimationFrame` schedule/cancel ref behavior from `ExpertEditPanelView` into `expertEditInteractionUtils`.
2. Keep session-state dispatch semantics and cleanup behavior unchanged.
...
4. `docs/planning/evidence/lane-b/2026-03-17-b2-01-expert-edit-seam-14.md`
...
1. `ExpertEditPanelView.tsx`: `5226` -> `5219` (`-7` lines, size-budget script count).
2. `expertEditInteractionUtils.ts`: `201` -> `230` (`+29` lines) with reusable RAF ref helpers.
3. Coupling reduction:
   - Session dispatch RAF ref scheduling/cancel behavior moved into interaction utility.
   - Both effect cleanup and no-callback dispatch paths in `ExpertEditPanelView` now reuse shared RAF ref helpers.

## Parity Assertions
1. `ExpertEditPanelView` prop contract remained unchanged.
2. No API/server/schema changes.
3. Session-state dispatch timing and cleanup behavior remained unchanged; extraction-only seam.
...
## Rollback Note
1. Revert this slice commit to inline session dispatch RAF scheduling/cancel behavior back into `ExpertEditPanelView`.
