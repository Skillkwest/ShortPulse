...
- `seedream-5-lite-per-image`: $0.035 per image, converted with shared rounding (`rawCredits = ceil(usd/0.01)`, `credits = ceil(rawCredits/5)*5`) for a current billed value of 5 credits; used by `fal-ai/bytedance/seedream/v5/lite/text-to-image` and `fal-ai/bytedance/seedream/v5/lite/edit`.
