...
6. `Refresh` reruns the live admin API read. No manual entry is required for storage bytes; egress remains only as fresh as the latest provider snapshot or configured environment values.
