...
20. **Use the launch-week production branch deliberately.** ShortPulse launched on `2026-07-07`; during active launch-week production operations, `production` is the only local and GitHub branch for repo work. Keep `git config --local shortpulse.allowedBranch` set to `production`, do not create or promote feature/staging branches, and never push directly to `main`. See `docs/launch-week-production-operations.md`.
...
25. **Use the production URL and database for launch-week work.** Browser/manual validation targets `https://www.shortpulse.ai`, and operational investigation uses the production Supabase project/database. Do not open or rely on `localhost`, `127.0.0.1`, arbitrary preview URLs, local browser sessions, or local database state unless the user explicitly asks for a local or non-production run. Local tests and commands can validate implementation details, but they do not prove deployed production behavior.
26. **Do not optimize for mobile right now.** ShortPulse is currently desktop-first for build and QA. Do not spend time on mobile-specific layouts, breakpoints, touch-only interactions, or mobile polish unless the user explicitly approves mobile scope in the current thread. Preserve basic responsive integrity when touching shared styles, but do not treat mobile as a launch target or validation requirement.
...
- **Frontend startup** (new terminal):
  ...
  npm run dev
  ```
- Use the frontend startup command only when the user explicitly asks for local development or a local repro. Launch-week browser validation should use `https://www.shortpulse.ai`.
