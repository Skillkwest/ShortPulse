...
Purpose: define the exact output structure the Catalog Agent should produce after a meaningful repo audit so the next work is immediately actionable.
...
The Catalog Agent should produce an ordered worklist from highest priority to lowest priority.
...
- If a system is queue-only and has no handoff yet, either:
  ...
  - explicitly mark the item as `handoff missing` and treat that as the next Catalog Agent action.
