...
function ReferenceGridComponent({
  ...
  const previousSelectorOutputIdsRef = React.useRef<string[] | null>(null);
  const previousCuratedOutputIdsRef = React.useRef<string[] | null>(null);
  const previousVisibleOutputIdsRef = React.useRef<string[] | null>(null);
  const previousVisibleCuratedOutputIdsRef = React.useRef<string[] | null>(null);
  const previousNearViewportOutputIdsRef = React.useRef<string[] | null>(null);
  const previousVisibleOutputByIdRef = React.useRef<Record<string, StudioOutput> | null>(null);
  const previousImageHydrationStateRef = React.useRef(imageHydrationState);
  const previousLoadedMapRef = React.useRef(loadedMap);
  const previousAutoplayEnabledIdsRef = React.useRef<string[] | null>(null);
  const previousPreviewSwapMetricsRef = React.useRef(previewSwapMetrics);
  const previousVirtualMetricsRef = React.useRef(virtualMetrics);
  const previousCuratedVirtualMetricsRef = React.useRef(curatedVirtualMetrics);
  const previousCanvasDropModeRef = React.useRef(canvasDropMode);
  const previousArchivePanelOpenRef = React.useRef(isArchivePanelOpen);

  React.useEffect(() => {
    const previous = previousSelectorOutputIdsRef.current;
    if (previous && !areStringArraysEqual(previous, allOutputIds)) {
      incrementFreezeInvestigationCounter("referenceGrid.internalChange.allOutputIds");
    }
    previousSelectorOutputIdsRef.current = allOutputIds;
  }, [allOutputIds]);

  React.useEffect(() => {
    const previous = previousCuratedOutputIdsRef.current;
    if (previous && !areStringArraysEqual(previous, curatedOutputIds)) {
      incrementFreezeInvestigationCounter("referenceGrid.internalChange.curatedOutputIds");
    }
    previousCuratedOutputIdsRef.current = curatedOutputIds;
  }, [curatedOutputIds]);

  React.useEffect(() => {
    const previous = previousVisibleOutputIdsRef.current;
    if (previous && !areStringArraysEqual(previous, visibleOutputIds)) {
      incrementFreezeInvestigationCounter("referenceGrid.internalChange.visibleOutputIds");
    }
    previousVisibleOutputIdsRef.current = visibleOutputIds;
  }, [visibleOutputIds]);

  React.useEffect(() => {
    const previous = previousVisibleCuratedOutputIdsRef.current;
    if (previous && !areStringArraysEqual(previous, visibleCuratedOutputIds)) {
      incrementFreezeInvestigationCounter("referenceGrid.internalChange.visibleCuratedOutputIds");
    }
    previousVisibleCuratedOutputIdsRef.current = visibleCuratedOutputIds;
  }, [visibleCuratedOutputIds]);

  React.useEffect(() => {
    const previous = previousNearViewportOutputIdsRef.current;
    if (previous && !areStringArraysEqual(previous, nearViewportOutputIds)) {
      incrementFreezeInvestigationCounter("referenceGrid.internalChange.nearViewportOutputIds");
    }
    previousNearViewportOutputIdsRef.current = nearViewportOutputIds;
  }, [nearViewportOutputIds]);

  React.useEffect(() => {
    const previous = previousVisibleOutputByIdRef.current;
    if (previous && !areOutputIdMapsEqual(previous, visibleOutputById)) {
      incrementFreezeInvestigationCounter("referenceGrid.internalChange.visibleOutputById");
    }
    previousVisibleOutputByIdRef.current = visibleOutputById;
  }, [visibleOutputById]);

  React.useEffect(() => {
    const previous = previousImageHydrationStateRef.current;
    if (previous !== imageHydrationState) {
      incrementFreezeInvestigationCounter("referenceGrid.internalChange.imageHydrationState");
    }
    previousImageHydrationStateRef.current = imageHydrationState;
  }, [imageHydrationState]);

  React.useEffect(() => {
    const previous = previousLoadedMapRef.current;
    if (previous !== loadedMap) {
      incrementFreezeInvestigationCounter("referenceGrid.internalChange.loadedMap");
    }
    previousLoadedMapRef.current = loadedMap;
  }, [loadedMap]);

  React.useEffect(() => {
    const previous = previousAutoplayEnabledIdsRef.current;
    if (previous && !areStringArraysEqual(previous, autoplayEnabledIds)) {
      incrementFreezeInvestigationCounter("referenceGrid.internalChange.autoplayEnabledIds");
    }
    previousAutoplayEnabledIdsRef.current = autoplayEnabledIds;
  }, [autoplayEnabledIds]);

  React.useEffect(() => {
    const previous = previousPreviewSwapMetricsRef.current;
    if (previous !== previewSwapMetrics) {
      incrementFreezeInvestigationCounter("referenceGrid.internalChange.previewSwapMetrics");
    }
    previousPreviewSwapMetricsRef.current = previewSwapMetrics;
  }, [previewSwapMetrics]);

  React.useEffect(() => {
    const previous = previousVirtualMetricsRef.current;
    if (previous !== virtualMetrics) {
      incrementFreezeInvestigationCounter("referenceGrid.internalChange.virtualMetrics");
    }
    previousVirtualMetricsRef.current = virtualMetrics;
  }, [virtualMetrics]);

  React.useEffect(() => {
    const previous = previousCuratedVirtualMetricsRef.current;
    if (previous !== curatedVirtualMetrics) {
      incrementFreezeInvestigationCounter("referenceGrid.internalChange.curatedVirtualMetrics");
    }
    previousCuratedVirtualMetricsRef.current = curatedVirtualMetrics;
  }, [curatedVirtualMetrics]);

  React.useEffect(() => {
    if (previousCanvasDropModeRef.current !== canvasDropMode) {
      incrementFreezeInvestigationCounter("referenceGrid.internalChange.canvasDropMode");
    }
    previousCanvasDropModeRef.current = canvasDropMode;
  }, [canvasDropMode]);

  React.useEffect(() => {
    if (previousArchivePanelOpenRef.current !== isArchivePanelOpen) {
      incrementFreezeInvestigationCounter("referenceGrid.internalChange.archivePanelOpen");
    }
    previousArchivePanelOpenRef.current = isArchivePanelOpen;
  }, [isArchivePanelOpen]);
