-- Fix linked-schema SQL lint issues surfaced by hosted Supabase lint:
-- 1) list_admin_model_usage_stats() had an ambiguous model_id reference because
--    plpgsql output-column variables shadowed query aliases.
-- 2) rollback_model_pricing_policy() selected too few columns into a %rowtype
--    variable, which triggers a lint warning.
...
begin
    ...
    select
        coalesce(generation_stats.model_key, click_stats.model_key) as model_id,
        coalesce(click_stats.generate_clicks_count, 0::bigint) as generate_clicks,
        coalesce(click_stats.generate_clicks_last_24h_count, 0::bigint) as generate_clicks_last_24h,
        coalesce(click_stats.generate_clicks_last_7d_count, 0::bigint) as generate_clicks_last_7d,
        coalesce(generation_stats.generations_started_count, 0::bigint) as generations_started,
        coalesce(generation_stats.generations_last_24h_count, 0::bigint) as generations_last_24h,
        coalesce(generation_stats.generations_last_7d_count, 0::bigint) as generations_last_7d,
        coalesce(generation_stats.successful_generations_count, 0::bigint) as successful_generations,
        coalesce(generation_stats.failed_generations_count, 0::bigint) as failed_generations,
        coalesce(generation_stats.pending_generations_count, 0::bigint) as pending_generations,
        coalesce(generation_stats.running_generations_count, 0::bigint) as running_generations,
        coalesce(generation_stats.unique_generation_users_count, 0::bigint) as unique_generation_users,
        coalesce(click_stats.unique_click_users_count, 0::bigint) as unique_click_users,
        click_stats.last_generate_click_time as last_generate_click_at,
        generation_stats.last_generation_time as last_generation_at
    from generation_stats
    full outer join click_stats
      on click_stats.model_key = generation_stats.model_key
    order by
        coalesce(generation_stats.generations_started_count, 0::bigint) desc,
        coalesce(click_stats.generate_clicks_count, 0::bigint) desc,
        generation_stats.last_generation_time desc nulls last,
        click_stats.last_generate_click_time desc nulls last,
        coalesce(generation_stats.model_key, click_stats.model_key) asc
    limit v_limit;
end;
$$;
