...
5. Prompt step always routes through the same `PromptStep` contract. Standard Create no longer exposes a chat-off inline-generate action; generation runs from the surface-owned primary Generate control, and assistant output must be applied explicitly before it can drive generation.
...
- Manual smoke in `/ai-studio`:
  ...
  6. Switching `Pulse -> Standard -> Pulse` clears the prior hidden Pulse transcript/workflow runtime and does not show Pulse messages or Pulse composer drafts in Standard mode.
