...
| first media paint timing | `frontend/features/media-library/hooks/useMediaPreviewRuntime.ts`, `frontend/features/ai-studio/components/MediaLibraryModal.tsx` | Trusted with scope limit | Route and modal emit first-media paint today; panel, file modal, panel preview modal, reference-grid, character-grid, and detail-modal do not | Restrict timing baseline to route/modal and document exclusions explicitly |
...
| first media paint timing | `frontend/features/media-library/hooks/useMediaPreviewRuntime.ts`, `frontend/features/ai-studio/components/MediaLibraryModal.tsx` | `media-library-route`, `media-library-modal` | Tied to image/video load handlers |
