...
- `Libraries -> Elements` is a first-class left-panel tool (`elements`) in AI Studio. It lands on a manage-first embedded Elements Library view without workflow tab chrome, then shows `Manage Elements` and `Element Profile` tabs only while profile mode is active. New elements stage locally and only persist after `Save Element`, with profile-photo/reference actions save-gated until the first explicit save completes.
