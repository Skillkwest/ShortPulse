...
| Agent client runtimes         | `frontend/features/ai-studio/createRuntime/useStandardCreateAgentRuntime.ts`, `frontend/features/ai-studio/createRuntime/usePulseCreateAgentRuntime.ts`, `frontend/features/ai-agent/useStandardCreateAgent.ts`, `frontend/features/ai-agent/usePulseCreateAgent.ts`                                                                                                                                                                                                                                       | Own lane-scoped chat state, attachment prep, prompt application, and response parsing. Active Create must not use the legacy shared bridge.                                  |
...
12. Active workspace prompt writes use the mounted mode-owned prompt setter. A Pulse draft can remain in Pulse state, but it must not appear in Standard composer input, Standard prompt state, Standard route requests, Standard snapshots, or Standard telemetry.
...
- Manual smoke in `/ai-studio`:
  ...
  8. Switching `Pulse -> Standard -> Pulse` clears the prior hidden Pulse transcript/workflow runtime and does not show Pulse messages or Pulse composer drafts in Standard mode.
