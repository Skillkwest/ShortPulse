...
- `frontend/components/AppMessage.tsx` is the canonical non-blocking feedback primitive for user-facing errors, warnings, informational notices, success banners, inline alerts, and toast-style local status messages.
- Use `AppMessage` for rendered message presentation instead of adding new route-local alert, banner, or toast wrappers.
- Keep domain copy normalization in the existing copy helpers. `AppMessage` owns presentation, tone, accessibility defaults, optional CTA/dismiss affordances, and layout stability; it does not own provider, auth, quota, billing, or media-specific message wording.
- Keep `frontend/components/ConfirmationModal.tsx` as the blocking confirmation dialog primitive.
