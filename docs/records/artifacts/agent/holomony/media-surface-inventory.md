...
| Reference Grid                   | `reference-grid`                   | `candidate-active-incident`   | `ai-studio-reference-grid`                                                 | `frontend/features/ai-studio/components/ReferenceGrid.tsx`, `frontend/features/ai-studio/reference-grid/controllers/useReferenceGridRuntimeScaffold.ts`, `frontend/features/ai-studio/reference-grid/controllers/useReferenceGridResolvedMediaController.ts`, `frontend/features/ai-studio/reference-grid/controllers/useReferenceGridImageHydrationController.ts`, `frontend/lib/mediaPerfTelemetry.ts`                                                                                                                                                                                                                                        | existing reference-grid perf audit tooling, runtime telemetry, adaptive-media audits; checked-in production browser perf audit path now enters `?perfAuditRuntime=1` automatically | wrong-asset prevention, autoplay/attach budget stability, drag/drop correctness, detail-preview correctness, archive/restore trust                                                                                                                                                       | user approval explicit; audited onboarding plan retained in `reports/current/2026-05-25-reference-grid-onboarding-plan.md`; first production baseline archived as historical evidence in `reports/archive/2026-05-25-reference-grid-production-baseline.md`; old `no clear blocker` classification is superseded by later user-visible production slow/broken-media incidents                                                            | complete the narrow Reference Grid incident lane: verify restore/signing/hydration and stale media authority fixes against fresh production evidence, then stop or pivot based on proof |
...
- These are valid Holomony targets when user-visible production evidence supports the lane.
- `reference-grid` has explicit user approval, a retained onboarding plan, and a retained historical first production baseline.
...
- `quick-slot-inventory` still requires its own explicit approval and retained onboarding packet.
...
5. Reference Grid incident lane when fresh production evidence shows slow or broken media
