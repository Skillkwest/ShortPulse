...
## Decision
- Use a single API route (`POST /api/ai/studio-agent`) that calls a pluggable chat-completions model with vision support (default: OpenAI `gpt-4.1`/`gpt-4.1-mini` with images via `image_url`).
