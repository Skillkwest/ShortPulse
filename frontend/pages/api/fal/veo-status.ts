/**
 * Proxies Fal.ai Veo 3.1 text-to-video status + result fetch.
 * Accepts { requestId }, returns status as-is, and fetches the result when completed.
 */
import type { NextApiRequest, NextApiResponse } from "next";

const FAL_VEO_STATUS_URL = "https://queue.fal.run/fal-ai/veo3.1/requests";
  ...
  try {
    ...
    if (!contentType || !contentType.includes("application/json")) {
      ...
      return res.status(500).json({
        error: "Fal Veo returned non-JSON response",
    ...
    const resultResp = await fetch(`${FAL_VEO_STATUS_URL}/${requestId}`, {
      method: "GET",
      headers: { Authorization: `Key ${apiKey}` },
      signal: controller.signal,
    });
    ...
    return res.status(500).json({ error: "Fal Veo status failed", detail: String(error) });
  } finally {
