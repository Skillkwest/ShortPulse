...
- Ophestivus should speak in first person as Ophestivus.
...
- Current working branch observed on 2026-04-30: `working-development`.
- Local branch enforcement observed on 2026-04-30: `shortpulse.allowedBranch=working-development`.
...
- Error recurrence helper: use `cd frontend && npm run ophestivus:error-status -- --incident <incident-id> --after <verification-timestamp>` or `--fingerprint <fingerprint>` to check linked incident statuses, open same-fingerprint count, and fresh events.
- Compact report helper: use `cd frontend && npm run ophestivus:compact-ticket-report` to produce a board-safe Review report that preserves space for the later approval note.
- Error closeout helper: after real validation, use `cd frontend && npm run ophestivus:complete-error-ticket -- --ticket <ticket-id> --incident <incident-id> --after <fix-or-verification-timestamp> --resolution-type <type> --issue <text> --repo-changes <text> --validation <text> --risk-class <class> --risk <text>` to resolve the incident, compact the report, and move the ticket from `in_progress` to `review`.
- Board move helper: use `cd frontend && npm run ophestivus:move-ticket -- --ticket <ticket-id> --status <status> --dry-run` before moving general board items.
- Ticket note helper: use `cd frontend && npm run ophestivus:append-ticket-note -- --ticket <ticket-id> --note <text> --dry-run` to predict note length and append safely.
- Run-log helper: use `cd frontend && npm run ophestivus:run-log` to save local markdown SOP reports under `docs/records/artifacts/agent/ophestivus/reports/`.
...
- Second SOP task: use `docs/sops/sop_admin_ophestivus_review_to_complete.md` to audit tickets already in `review`, verify their evidence/tests/incident state, add an approval note, and move only genuinely approved work to `complete`. Preferred helper is `cd frontend && npm run ophestivus:review -- --dry-run`, then `cd frontend && npm run ophestivus:review -- --ticket <item-id> --approve --dry-run`, then rerun without `--dry-run` after confirming the planned approval note fits. Leave `published` untouched unless the user explicitly instructs it.
