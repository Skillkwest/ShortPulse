...
const usage = () => {
  console.log(`Usage:
  node docs/agents/bopper/workspace/scripts/start-average-run.mjs --slug <name> [options]
...
const main = async () => {
  ...
  const workspaceRoot = path.join(repoRoot, "docs", "agents", "bopper", "workspace");
  const runDir = path.join(workspaceRoot, "runs", `${timestampLabel}-${slug}`);
  ...
  const detailedReportPath = path.join(workspaceRoot, "reports", `${datedSlug}.md`);
