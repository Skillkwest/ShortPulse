create table if not exists public.create_pulse_builtin_runtime (
    singleton boolean primary key default true check (singleton = true),
    pulse_definitions jsonb not null,
    updated_at timestamptz not null default now(),
    updated_by_user_id uuid null,
    updated_by_email text null,
    constraint create_pulse_builtin_runtime_definitions_array_check check (
        jsonb_typeof(pulse_definitions) = 'array'
    )
);

alter table public.create_pulse_builtin_runtime enable row level security;

insert into public.create_pulse_builtin_runtime (
    singleton,
    pulse_definitions,
    updated_by_email
)
values (
    ...
    jsonb_build_array(
        jsonb_build_object(
            ...
            'systemInstructions', $$SYSTEM INSTRUCTIONS — “Universal Single-Shot Video Prompt Director (I2V-Optimized)”
...
- Maintain continuity: do not change outfit, age, hairstyle, identity, or location mid-shot.$$,
        ...
        jsonb_build_object(
            ...
            'artifactTarget', 'video_prompt',
            'systemInstructions', $$SYSTEM INSTRUCTIONS — “Cut-Scene Director (Multi-Shot Video Model)”
...
- No graphic violence, sexual content, hate, or illegal wrongdoing instruction. If the user requests disallowed content, politely refuse and offer a toned-down alternative.$$,
        ...
        jsonb_build_object(
            ...
            'systemInstructions', $$You are **Story Circle Scene-Prompt GPT**. You run a strict step-by-step workflow to turn uploaded character images into a refined story and then output perfect **image prompts** (images only, no video). Ask **one question at a time** and **advance only after the current step is completed**.
...
revoke all on table public.create_pulse_builtin_runtime from public;
revoke all on table public.create_pulse_builtin_runtime from anon;
revoke all on table public.create_pulse_builtin_runtime from authenticated;
grant all on table public.create_pulse_builtin_runtime to service_role;
