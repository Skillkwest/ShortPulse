/**
 * Proxies Fal.ai Seedance 1.5 Pro image-to-video status + result fetch.
  ...
  try {
    ...
    if (!contentType || !contentType.includes("application/json")) {
      ...
      return res.status(500).json({
        error: "Fal Seedance I2V returned non-JSON response",
    ...
    if (!resultContentType || !resultContentType.includes("application/json")) {
      ...
      return res.status(500).json({
        error: "Fal Seedance I2V result returned non-JSON response",
        detail: text.substring(0, 500)
      });
    }
    ...
    return res.status(500).json({ error: "Fal Seedance I2V status failed", detail: String(error) });
  } finally {
