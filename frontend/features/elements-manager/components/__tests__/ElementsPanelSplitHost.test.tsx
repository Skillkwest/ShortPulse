...
import { ElementsPanelSplitHost } from "../ElementsPanelSplitHost";
...
vi.mock("../../../ai-studio/components/ElementsEmbeddedMediaLibraryPanel", () => ({
  ElementsEmbeddedMediaLibraryPanel: (props: Record<string, unknown>) => {
    ...
    return <div data-testid="elements-bottom-media-library" />;
  },
}));

vi.mock("../ElementsManagerShell", () => ({
  ElementsManagerShell: (props: Record<string, unknown>) => {
    workspaceSpy(props);
    return <div data-testid="elements-top-workspace" />;
  },
}));

describe("ElementsPanelSplitHost", () => {
  it("renders the Elements workspace above the embedded media library", () => {
    ...
    expect(screen.getByTestId("elements-top-workspace")).toBeInTheDocument();
    expect(screen.getByTestId("elements-bottom-media-library")).toBeInTheDocument();
    expect(screen.queryByRole("separator")).not.toBeInTheDocument();
    expect(container.querySelector(".elements-panel-top-section")).not.toBeNull();
    const bottomSection = container.querySelector(
      ".elements-panel-bottom-section"
  ...
  it("keeps the embedded media library in assignment mode without click-selection wiring", () => {
    render(<ElementsPanelSplitHost projectId="project-123" />);
