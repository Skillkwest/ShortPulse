# SOP: Admin Kamaji Board Operations

Purpose: operate the shared admin kanban board safely as the durable task source for admins and the future Kamaji steward.
...
- Current phase: human/admin task tracking only. Autonomous Kamaji execution is not enabled by this SOP.
...
- Architecture decision: `docs/adr/0072-admin-kanban-and-kamaji-foundation.md`
...
5. Use the activity route for future detail/timeline views when audit context is needed.
...
- Keep `Published` human-controlled until a later approved Kamaji automation phase changes the contract.
