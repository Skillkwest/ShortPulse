...
| 09 | In Progress | Engineering | Phase 08 complete | Admin transitions/access correctness proven | `evidence/unified-buildout/phase-09/` | Slice A complete: atomic admin incident status RPC migration (`039`) added and `/api/admin/errors-status` now delegates to RPC for transactional status/event promotion behavior. Slice B complete: lifecycle edge coverage expanded (invalid-input mapping, array payload normalization, malformed payload fail-safe, exception logging path) and API docs updated for timestamp semantics. Slice C reliability patch complete: `/api/admin/error-events` actionable filter no longer issues invalid relation logic trees; route now returns bounded merged actionable results with degraded-health guidance instead of `500` parse failures. Awaiting human phase-exit checkpoint before status promotion. |
| 10 | In Progress | Engineering | Phase 09 complete | Residual security controls verified in staging | `evidence/unified-buildout/phase-10/` | Slice A complete: bounded webhook raw-body readers landed with explicit `413` responses (`/api/billing/stripe/webhook`, `/api/fal/webhook`) and webhook `500` error-surface hardening (sanitized generic failures). Slice B complete: describe-image trusted-host policy now fail-closed for non-allowlisted external hosts, with route coverage and SOP/API env contract updates. Remaining scope: runtime SQL security audit integration + phase-exit checkpoint. |
