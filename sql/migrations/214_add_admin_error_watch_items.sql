-- Add structured Admin Errors watch markers to the canonical status RPC.
-- The lifecycle status remains open/resolved/ignored; watch state is metadata
-- on the resolved status-history entry and current incident metadata.
...
create or replace function public.admin_update_app_error_status(
    ...
    p_watch_item boolean default false
)
returns jsonb
language plpgsql
security definer
...
declare
    ...
    v_watch_item boolean := coalesce(p_watch_item, false);
...
begin
    ...
    if v_watch_item and v_next_status <> 'resolved' then
        raise exception 'watch items must use resolved status.' using errcode = '22023';
    end if;
        ...
        if v_event.incident_id is not null then
            ...
            v_history_entry := jsonb_strip_nulls(
                jsonb_build_object(
                    ...
                    'note', v_note,
                    'watch_item', case when v_watch_item then true else null end
                )
            );
            ...
            if v_next_status = 'resolved' then
                ...
                if v_watch_item then
                    v_metadata := v_metadata || jsonb_strip_nulls(
                        jsonb_build_object(
                            'watch_item', true,
                            'watch_note', v_note,
                            'watch_marked_at', v_now,
                            'watch_marked_by', p_admin_user_id,
                            'watch_marked_by_email', p_admin_user_email
                        )
                    );
                end if;
