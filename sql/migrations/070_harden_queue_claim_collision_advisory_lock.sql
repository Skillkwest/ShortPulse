-- Queue claim collision hardening: use a per-user advisory xact lock during claim
-- selection so concurrent claimers cannot select different rows for the same user
-- and then collide on ux_ai_generation_submit_queue_dispatching_user.
...
begin
    ...
    with lockable as (
        ...
        where (
          ...
          and pg_try_advisory_xact_lock(hashtext('generation_submit_queue_claim:' || q.user_id::text))
