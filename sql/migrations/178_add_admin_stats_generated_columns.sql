-- Add DB-maintained scalar projections for the admin global stats RPC so
-- /api/admin/stats/global can preserve its payload contract without repeatedly
-- reading TOAST-heavy generation JSON fields on every dashboard refresh.

alter table public.ai_generations
    add column if not exists admin_stats_autosave_decision text
    generated always as (metadata->>'autosave_decision') stored;

alter table public.generation_projection
    add column if not exists admin_stats_style_applied boolean
    generated always as (
        case
    ...
    add column if not exists admin_stats_character_mode_applied boolean
    generated always as (
        case
    ...
    add column if not exists admin_stats_has_reference_inputs boolean
    generated always as (
        case
...
begin
    ...
    projection_generation_rows as materialized (
        select
            ...
            g.mode,
            g.created_at,
            gp.admin_stats_style_applied as style_applied,
            gp.admin_stats_character_mode_applied as character_mode_applied,
            gp.admin_stats_has_reference_inputs as has_reference_inputs
    ...
    autosave_summary as (
        select
            count(*) filter (where admin_stats_autosave_decision = 'auto_persisted')::bigint as auto_persisted_total,
            count(*) filter (
                where admin_stats_autosave_decision = 'auto_persisted'
            ...
            count(*) filter (where admin_stats_autosave_decision = 'autosave_skipped')::bigint as autosave_skipped_total,
            count(*) filter (
                where admin_stats_autosave_decision = 'autosave_skipped'
                  and created_at >= now() - interval '24 hours'
