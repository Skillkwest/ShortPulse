...
describe("useAiStudioWorkspaceActions", () => {
  ...
  it("preserves non-workflow tool selections like templates and presets", () => {
    const setSelectedTool = vi.fn();
    const setMode = vi.fn();
    const { result } = renderHook(() =>
      useAiStudioWorkspaceActions(
    ...
    act(() => {
      ...
      result.current.handleToolSelect("presets");
    });

    expect(setSelectedTool).toHaveBeenCalledWith("templates");
    expect(setSelectedTool).toHaveBeenCalledWith("presets");
  ...
  it("always routes reference-grid file picker uploads to outputs and clears input value", () => {
    ...
    expect(addOutputsFromFiles).toHaveBeenLastCalledWith(expect.any(Object), "drop");
    ...
    expect(addOutputsFromFiles).toHaveBeenCalledTimes(5);
