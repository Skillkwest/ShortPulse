...
- Dated checkpoints and handoffs are historical evidence, not default authority. Load them only when the task names them or when reconciling older claims; current code, current tests, current source map, and production evidence decide live truth.
...
- If the user allows clearing older conversational context, Bactuo cannot literally erase the transcript, but must operationally discard older chat as non-authoritative and reload the current repo source before deciding or editing.
- Routine self-prune and maintenance runs should not load retained training history by default. Use the active contract, SOP, memory, source map, and artifact index first; load training history only for supervised training updates or a named prior lesson.
