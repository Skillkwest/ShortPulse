...
- Customer-facing subscription changes use `/api/billing/subscription/change`. Existing owned Stripe subscriptions must not create a second subscription through new Checkout. Same-interval higher-plan upgrades and safe single-item monthly-to-annual higher-plan upgrades use an immediate paid Stripe subscription update with `payment_behavior = pending_if_incomplete`, `proration_behavior = always_invoice`, and `billing_cycle_anchor = now`; local entitlement changes and credit grants must wait for Stripe to accept payment and emit the paid invoice/subscription webhooks. Subscriptions that also carry recurring storage add-on items may directly update only the base plan item for compatible same-interval higher-plan upgrades while preserving the storage add-on item for webhook projection. Multi-item downgrades and billing-interval changes remain blocked unless the active storage add-on is removed or changed first, so the route does not silently bypass Portal scheduling behavior. The route validates the target Stripe price amount and interval against `billing_plan_offers` before creating Checkout/Portal sessions or mutating the base plan item.
- Stripe Billing Portal configuration remains part of the customer-facing subscription-change contract for single-item existing subscriptions that are not immediate paid higher-plan upgrades. `features.subscription_update.products[*].prices` must include every active monthly and annual paid-plan price that `/api/billing/subscription/change` can target through Portal, and interval-shortening changes such as annual to monthly must be scheduled at period end through the portal `schedule_at_period_end` conditions to match the `/profile` downgrade language. Portal update flows must not be used for subscriptions with recurring storage add-on items because Stripe rejects multi-item subscription updates through that surface.
...
1. Confirm the Stripe invoice is paid and its `billing_reason` is `subscription_create`, `subscription_cycle`, or a ShortPulse-marked immediate-upgrade `subscription_update`.
...
6. Verify `/profile?section=subscription` still routes each plan card to the intended self-serve flow:
   ...
   - Stripe-managed same-interval paid upgrades should charge immediately through Stripe, then rely on the paid invoice webhook for the upgraded monthly credit allocation
...
- Generic subscription change/proration invoices must not mint an extra monthly credit grant. A `subscription_update` invoice may grant the target monthly allocation only when it is paid and marked as ShortPulse's immediate paid upgrade path.
