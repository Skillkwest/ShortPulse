/**
 * Proxies Fal.ai FLUX 2 PRO status + result fetch.
 * Accepts { requestId }, fetches status first, then result when ready.
 */
import type { NextApiRequest, NextApiResponse } from "next";

const FAL_FLUX2PRO_STATUS_URL = "https://queue.fal.run/fal-ai/flux-2-pro/requests";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const statusResp = await fetch(`${FAL_FLUX2PRO_STATUS_URL}/${requestId}/status`, {
      method: "GET",
    ...
    if (!contentType || !contentType.includes("application/json")) {
      ...
      return res.status(500).json({
        error: "Fal FLUX 2 PRO returned non-JSON response",
    ...
    // Fetch result
    const resultResp = await fetch(`${FAL_FLUX2PRO_STATUS_URL}/${requestId}`, {
      method: "GET",
      headers: { Authorization: `Key ${apiKey}` },
      signal: controller.signal,
    });
    ...
    if (!resultContentType || !resultContentType.includes("application/json")) {
      ...
      return res.status(200).json({
        status: "error",
        state: "error",
        error: "Fal FLUX 2 PRO result returned non-JSON response",
        detail: text.substring(0, 500),
        request_id: requestId,
      });
    }
    ...
    return res.status(200).json({
      status: "error",
      state: "error",
      error: "Fal FLUX 2 PRO status check failed",
      detail: String(error),
      request_id: requestId,
    });
  } finally {
