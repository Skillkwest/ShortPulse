...
- In scope: safety policy profile selection, server-authoritative pre-provider input gating for both Create agent routes and Fal submit routes, the shared Safe Completion Contract and bounded model-refusal recovery, image preflight gating for retained image-analysis routes, client pre-send gating for guided studio-agent UX, admin control-plane API usage, runtime env tuning knobs, SQL diagnostics, and rollback actions.
- Out of scope: editable agent prompt authoring, provider onboarding, downstream generation-provider safety settings, and non-agent route behavior.

## Control Surface Summary

The safety control surface has seven layers:
...
- Current Create contract: both Standard and Pulse use the client mirror for the provider-bound payload while preserving raw visible transcript text. The server remains authoritative.

4. Safe Completion contract and bounded recovery.

- Files:
  - `frontend/features/agent-runtime/studioAgentSafeCompletion.ts`
  - `frontend/features/agent-runtime/studioAgentSafetyRuntimeConfig.ts`
  - mode-owned Standard/Pulse runtimes
- Injects one versioned, non-editable platform instruction after editable agent behavior instructions.
- Allows exactly one internal recovery call only for an eligible model-authored refusal.
- Never recovers a policy refusal, production hard floor, ambiguous-age sexual signal, provider HTTP safety block, output-safety refusal, malformed output, or route/configuration error.
- `STUDIO_AGENT_SAFE_COMPLETION_ENABLED=false` disables this instruction and recovery for incident containment; it never disables safety enforcement.

5. Runtime output post-process (defense-in-depth).
...
6. Admin control plane (state and mutations).
...
- Both Create agent routes enforce input safety before provider execution when `STUDIO_AGENT_SAFETY_INPUT_PRECHECK_ENABLED=true` (default). Standard still forwards raw visible conversation turns and does not inherit Pulse workflow state; only its provider-bound safety payload is rewritten when policy requires it.
...
- Both Create client chat paths run a pre-send mirror gate when `NEXT_PUBLIC_STUDIO_AGENT_SAFETY_INPUT_PRECHECK_ENABLED=true` (default). Standard rewrites only the provider-bound copy and preserves raw visible history; the server remains authoritative.
...
## Safe Completion Contract

- Authority: ADR 0099 and `frontend/features/agent-runtime/studioAgentSafeCompletion.ts`.
- Applies to Standard, custom Pulse, and built-in guided workflows without mutating their live prompt records.
- Transformable content completes in the same assistant response using the smallest safe substitutions while preserving safe creative intent, format, continuity, and production constraints.
- The agent must not ask for an SFW resubmission, ask for permission, give a policy lecture, or merely offer a safer version later.
- Recovery requires an enabled server precheck and a non-refusal policy decision. Missing decision evidence fails closed.
- One eligible model-authored refusal may trigger one internal recovery attempt. A recovery refusal/error ends the turn; no second recovery or visible synthetic user message is allowed.
- Primary and recovered outputs pass the shared output-safety finalizer. A final refusal remains actionless and non-reusable.
- Editable Standard and Pulse instructions own role/workflow/output behavior below this platform contract and cannot disable it.
...
| `STUDIO_AGENT_SAFETY_INPUT_PRECHECK_ENABLED`                   | `true`                         | Enables the server pre-provider safety gate for both Create agent routes.                                                                                                                                                                             | Keep `true` in production. Disable only as emergency rollback while keeping output post-process enabled; recovery fails closed while the gate is off. |
...
| `STUDIO_AGENT_SAFE_COMPLETION_ENABLED`                         | `true`                         | Enables the shared Safe Completion instruction and one eligible model-refusal recovery.                                                                                                                                                               | Disable only for incident containment. This lever cannot disable input safety, hard floors, provider-block handling, or output safety.                |
...
- Exercise one safe prompt, one transformable mixed-content prompt, one eligible simulated model refusal, and one explicit hard-floor refusal per Create route.
- Confirm transformable work completes without a user resubmission, eligible recovery occurs at most once, and the hard-floor refusal makes no recovery call.
- Confirm telemetry fields: `safety_stage`, `category`, `decision_action`, `decision_source`, `provider_call_skipped`, `safe_completion_contract_version`, `safe_completion_enabled`, `refusal_source`, `recovery_eligible`, `recovery_attempted`, `recovery_outcome`, `recovery_skip_reason`, `recovery_latency_ms`.
...
Safe Completion telemetry is classification-only. Do not add raw prompts, outputs, user identifiers, emails, or reference URLs.
...
6. Do not use Safe Completion recovery for hard floors, policy refusals, provider HTTP safety blocks, output-safety refusals, malformed output, or infrastructure/configuration failures.
7. Do not implement recovery through legacy V2, a client-authored second turn, or an editable prompt override.
