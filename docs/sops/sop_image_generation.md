...
2. `OPENAI_API_KEY` is required for separate text/describe workflows documented in `docs/sops/sop_text_generation.md`; image Create/Edit generation does not use direct OpenAI image routes.
...
5. On click:
   ...
   - Fal/Kie image routes, including Kie GPT Image 2, reserve credits before provider submission (no immediate debit posted).
   ...
   - Fal/Kie task polling updates status; success stores `resultUrls`, sets `previewUrl`, and clears errors. Failures set `errorMessage` and stop polling.
...
- Resolution options are model-driven from `modelRegistry.ts` (`allowedResolutions` + `defaultResolution`):
  ...
  - Kie GPT Image 2 text-to-image and image-to-image: `1K`, `2K`, `4K`; the submit adapter normalizes provider-invalid combinations so `auto` submits `1K`, and requested `2K`/`4K` downgrades to `1K` for `5:4`, `4:5`, `3:1`, `1:3`, and `9:21`.
...
- Expert Edit panel contracts:
  ...
  6. Inpaint submit exports the visible image-area mask window (`imageRect`) against the same crop-frame output basis as base-image flatten so FLUX Fill mask pixels remain aligned with the exported frame.
  7. `Remove Background` submits the currently selected layer image only, routes regenerate through hidden Bria RMBG (`fal-ai/bria/background/remove`) with prompt-optional submit policy, and uses the shared model-pricing policy for the displayed estimate and final debit.
  8. Generate submit flattening (regular + inpaint base image) and manual `Flatten Layers` both use the same crop-frame flatten contract: aspect-aware output dimensions, transparent uncovered pixels, current layer transforms/z-order, and the canvas frame as the crop boundary. Outer stage viewport zoom/pan is editing chrome and must not change the flattened pixels.
  ...
  20. Viewport/camera contract is shared across inline + modal surfaces for editing interactions: camera state uses one `scale` plus normalized offsets (`offsetXRatio`, `offsetYRatio`) resolved per active surface bounds, modal stage geometry is aspect-fit (no modal-only square/frame path), and crop-frame flattening must preserve output aspect contracts without baking outer viewport zoom/pan into exported pixels.
  ...
  24. Inline stage transform ownership is split by contract: wrapper/shell owns camera transform state for editing zoom/pan, primary dropzone remains the pointer sampling and crop-frame surface for draw/transform/flatten tools, and transform-overlay affordances may render outside the image bounds while wrapper-level clipping prevents camera/image spill outside the stage frame.
