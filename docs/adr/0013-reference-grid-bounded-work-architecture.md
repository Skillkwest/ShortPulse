...
Existing direction in ADR 0009 established derivative-first delivery + virtualization, but the session-local Reference Grid still needed explicit bounded-work controls for 500-card smoothness targets.
...
- Keep the newest active references in the hot path (default cap: 500).
- Move older non-pinned, non-in-flight cards into an archived bucket with restore actions.
...
- Follow-ups:
  ...
  - Add perf-harness baselines for 100/300/500-card profiles.
