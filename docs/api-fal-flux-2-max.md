# Fal.ai FLUX 2 MAX Text-to-Image API Reference

Reference for integrating Fal.ai’s FLUX 2 MAX (`fal-ai/flux-2-max`) queue endpoint.
...
response=$(curl --request POST \
  --url https://queue.fal.run/fal-ai/flux-2-max \
  --header "Authorization: Key $FAL_KEY" \
  --header "Content-Type: application/json" \
  --data '{
    "prompt": "A lavish, baroque-style image ...",
...
```bash
curl --request GET \
  --url https://queue.fal.run/fal-ai/flux-2-max/requests/$REQUEST_ID/status \
  --header "Authorization: Key $FAL_KEY"
```
3) Result
```bash
curl --request GET \
  --url https://queue.fal.run/fal-ai/flux-2-max/requests/$REQUEST_ID \
  --header "Authorization: Key $FAL_KEY"
```
...
## Pricing (ShortPulse)
- Provider rate: **$0.07 for the first megapixel**, **$0.03 for each additional (rounded up) megapixel**.
- Credits are valued at $0.01 each.
- Calculation: `megapixels = (width * height) / 1_000_000`; `mpUnits = max(1, ceil(megapixels))`; `usd_raw = 0.07 + max(0, mpUnits - 1) * 0.03`; `credits = ceil(usd_raw / 0.01)` (minimum 1). Aspect → size mapping: 1:1=1024x1024, 4:3=1200x900, 3:4=900x1200, 16:9=1344x756, 9:16=756x1344.
...
## Notes
- Stream endpoint: `https://fal.run/fal-ai/flux-2-max/stream` is available if progressive responses are needed.
