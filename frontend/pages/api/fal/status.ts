/**
 * Proxies Fal.ai flux/dev status + result fetch.
 * Accepts { requestId }, fetches status first, then result when ready.
 */
import type { NextApiRequest, NextApiResponse } from "next";

const FAL_STATUS_URL = "https://queue.fal.run/fal-ai/flux/requests";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const statusResp = await fetch(`${FAL_STATUS_URL}/${requestId}/status`, {
      method: "GET",
    ...
    const resultResp = await fetch(`${FAL_STATUS_URL}/${requestId}`, {
      method: "GET",
      headers: { Authorization: `Key ${apiKey}` },
      signal: controller.signal,
    });
    ...
    return res.status(500).json({ error: "Fal status failed", detail: String(error) });
  } finally {
