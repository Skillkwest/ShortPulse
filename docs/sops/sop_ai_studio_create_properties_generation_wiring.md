...
| Workflow plan access          | `frontend/lib/billing/aiStudioWorkflowEntitlements.ts`, `frontend/features/ai-studio/routes/AiStudioRouteApp.tsx`, `frontend/lib/server/api/aiStudioWorkflowPlanGuard.ts`, `frontend/lib/server/api/falSubmitProxy.ts`, `frontend/pages/api/elevenlabs/*`                                                                                                                                                                             | Lets baseline/free accounts explore AI Studio Video and Sound while blocking generation, keeps Starter-restricted Video/Sound rail buttons visible as plan links that reveal `View plans` on hover/focus, and denies executable video/audio submits before billing reservation or provider dispatch. |
...
9. In Create `Pulse` mode, `PulseCreatePanelView` mounts `CreatePulsePresetPanel` in the left rail. Custom Pulse preferences are mounted through the Pulse-only `CreatePulsePreferenceProvider`, so Standard mode does not load saved custom Pulse definitions into active page state. That panel merges a shared catalog of published built-in guided workflows plus curated per-user custom Pulse allocation from `user_preferences`, supports bidirectional drag/drop with `CreatePulsePresetsSurface`, limits user editing to custom Pulses only, and activates a pinned preset by setting the current Pulse runtime selection instead of mutating the visible `agentInput` composer. `CreatePulsePresetsSurface` shows only Pulses that are not currently pinned in the left rail; dropping a rail Pulse back onto that surface removes it from the rail and returns it to the available catalog. `CreatePulsePresetsSurface` also serves as a direct activation surface: clicking a Pulse there activates it immediately and pins it into the rail so the active owner remains visible. The library panel (`Libraries -> Presets -> Pulses`) remains catalog-management only, can inspect built-in metadata, surfaces seeded-fallback catalog status when needed, and does not activate Pulses directly. Built-ins remain guided workflow compatibility records resolved from the admin control plane. Admin draft built-ins remain stored in `/admin/agent-instructions` but are excluded from `/api/ai/create-pulse-builtins`, the merged Create rail catalog, and built-in Pulse runtime starts until republished. User-authored custom Pulses are simpler: they are saved instruction presets whose runtime authority comes from `systemInstructions`, not from hidden workflow metadata. Retired Pulse metadata and built-in id collisions are discarded during preference normalization, and legacy custom Pulse records are normalized into the standalone `custom_gpt` contract instead of rehydrating hidden workflow fields. Built-in workflow runtime instructions are resolved authoritatively from the server control plane before `/api/ai/studio-agent-pulse` executes. Pulse is an agent-only lane: it does not read or own the Standard Create chat toggle, it uses the Pulse create runtime and `/api/ai/studio-agent-pulse`, it hides the inline chat-mode toggle, and it suppresses the shared `StylesControl` plus right-rail Styles toggle until the user switches back to `Standard`. Switching back to `Standard` now parks active Pulse runtime ownership instead of clearing it; returning to `Pulse` restores the parked session when preset/session authority is still valid.
10. Create-mode switching does not fork the shared workspace right rail. `Reference Grid`, `Quick Slot Inventory`, and `Canvas` remain page-global surfaces across `Standard` and `Pulse`, so mode switches keep one shared output focus and quick-slot/reference projection authority within the current workspace. Primary workflow shells such as Create, Edit, Video, and Sound may resize wide enough to collapse the right column; when the column is collapsed, the shared right-rail state remains authoritative but the heavy rail content is not mounted until width returns.
...
- Character mode in Create narrows to Character Mode-approved image-to-image-capable models, including Kie GPT Image 2 Edit and the paired Seedream/Nano Banana edit lanes. Direct OpenAI GPT Image 2 is retired, disabled, and intentionally non-discoverable; keep Kie GPT Image 2 text/edit chips selectable.
...
- In the Video Lip Sync lane, state effects pin the hidden internal Lip Sync model while the panel presents product-only controls (`Character image`, `Voice audio`, resolution, and generic faster-generation control). Generate guardrails require a character image and ready voice audio, block uploading/failed/stale local audio, enforce known audio-duration limits by selected resolution, and the submit path stages both character image and voice audio through `/api/fal/upload-url` so Fal OmniHuman receives verified Fal CDN URLs. App-owned Lip Sync media stages from storage/internal-ref authority when known; signed/display URLs are derived playback data and only remain the staging source for URL-only media.
...
- Enforce workflow-plan access before dispatch for Video/Sound entry points. Baseline/free users may navigate into Video/Sound for exploration, but their generation entry points must show the shared `View plans` CTA and must not dispatch. Starter users are not allowed to navigate into Video/Sound from the left rail; the left rail keeps the normal Video/Sound buttons in place as plan links and reveals `View plans` on hover/focus. The route-level server guard must reject restricted FAL video and ElevenLabs audio generation before billing reservation or provider dispatch.
...
- For the July 7 launch window, the manual reload action is launch-deferred and hidden by default through `NEXT_PUBLIC_AI_STUDIO_MANUAL_WORKFLOW_RELOAD_ENABLED`. Leave workflow reload metadata capture and persistence active while this action is disabled so project restore, saved generated-media detail, and post-launch re-enable work keep their durable context.
...
- Pulse preset click/apply in Create activates hidden Pulse runtime behavior on `/api/ai/studio-agent-pulse` instead of writing preset text into the visible Create composer. The Pulse Presets Library and Create Pulse rail share a merged catalog of published global built-in guided workflows from the admin control plane plus per-user custom Pulses from `user_preferences`. Built-in runtime instructions are re-resolved server-side before execution, draft built-ins are filtered out of runtime catalog reads, and `/api/ai/create-pulse-builtins` must distinguish normal seeded fallback from degraded control-plane fallback so Create/Pulse restore logic can reason about catalog authority correctly.
