...
- Signup intent approval is not billing entitlement. New Auth users still receive only the zero-value baseline-access profile from `handle_new_user_billing_setup`; `/api/account/bootstrap` may create or sync a real Stripe customer identity, but customer identity is not subscription state.
