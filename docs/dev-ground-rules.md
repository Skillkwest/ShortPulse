...
21. **Treat Seedance 2 as quarantined until explicit provider go-live.** `kie-ai/seedance-2.0` and `kie-ai/seedance-2.0-fast` are dormant scaffolding, not active product scope. Do not include them in default buildout, reliability hardening, rollout, validation, or support assumptions unless the current task explicitly reactivates them after provider approval. Keep existing code/docs only as implementation placeholders and avoid spending active-lane time on them unless they directly break live video paths.
