...
- Expert Edit panel contracts:
  ...
  12. `Libraries -> Presets` opens one primary left-panel Presets Library with internal `Pulses` and `Prompt Presets` sections. The `Pulses` section is backed by shared per-user Pulse persistence. Built-in starter Pulses are editable and save as per-user overrides on stable seeded ids, while custom user-authored Pulses can define either `prompt_editor` or `workflow_gpt` runtime behavior and also appear in Expert Create `Pulse` mode. The `Prompt Presets` section is populated from the full Expert Edit preset catalog (with overrides applied), keeps the right rail visible, and supports click-to-edit preset name/prompt in a modal.
