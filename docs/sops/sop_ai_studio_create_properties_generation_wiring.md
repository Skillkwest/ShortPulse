...
- Pulse preset click/apply in Expert Create activates hidden Pulse runtime behavior on `/api/ai/studio-agent-pulse` instead of writing preset text into the visible Create composer. The Pulse Presets Library and Create Pulse rail share a merged catalog of global built-in guided workflows from the admin control plane plus per-user custom Pulses from `user_preferences`. Built-in runtime instructions are re-resolved server-side before execution.
