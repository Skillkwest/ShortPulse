...
- AI Studio session persistence APIs: authenticated save/get/list routes under `/api/ai/sessions/*` provide server-side durable snapshot persistence (`POST /api/ai/sessions/save`, `GET /api/ai/sessions/:sid`, `GET /api/ai/sessions?limit=&cursor=`) behind `SHORTPULSE_AI_STUDIO_SESSIONS_API_ENABLED` (default enabled).
