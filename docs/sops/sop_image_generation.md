...
- Expert Edit panel contracts:
  ...
  12. `Libraries -> Presets` opens one primary left-panel Presets Library with internal `Pulses` and `Prompt Presets` sections. The `Pulses` section is backed by shared per-user Pulse persistence. The built-in starter set is exactly `Video Prompt Magic`, `Multi Sequence Video Prompt`, and `DFY Story Builder`; built-ins are editable and save as per-user overrides on stable seeded ids. Custom user-authored Pulses use the guided `workflow_gpt` runtime contract and also appear in Expert Create `Pulse` mode. The `Prompt Presets` section is populated from the full Expert Edit preset catalog (with overrides applied), keeps the right rail visible, and supports click-to-edit preset name/prompt in a modal.
