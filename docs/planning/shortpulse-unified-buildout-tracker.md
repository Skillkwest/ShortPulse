...
| 08 | In Progress | Engineering | Phase 07 complete | Stripe idempotency/recovery correctness proven | `evidence/unified-buildout/phase-08/` | Slice A complete: webhook event claim flow converted to insert-first idempotent claim semantics with duplicate conflict + non-conflict claim failure handling before side effects. Slice B replay safety landed: duplicate-claim deliveries now continue side effects and ledger duplicate source-ref conflicts are treated as idempotent no-op for retry safety. Slice B bootstrap safety landed: checkout/portal now share `ensureStripeCustomerForUser` and no-customer portal dead-end is removed. Remaining work: failed-event replay runbook details + broader billing state parity coverage. |
