/**
 * Proxies Fal.ai Veo 3.1 first/last frame submit requests.
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const upstream = await fetch(FAL_VEO_FIRST_LAST_SUBMIT_URL, {
      method: "POST",
      headers: {
    ...
    return res.status(500).json({ error: "Fal Veo first/last frame submit failed", detail: String(error) });
  } finally {
