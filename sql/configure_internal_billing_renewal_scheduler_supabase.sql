-- Configure Supabase Cron for internal-comp billing renewals.
-- Current default schedule in this script is hourly at minute 15 UTC.
-- This is an environment-level ops script (not a schema migration).
--
-- Secrets expected in Supabase Vault after setup:
--   1) shortpulse_internal_billing_renewals_run_url
--      Example: https://<deployment-domain>/api/internal/billing-contract-renewals/run
--   2) shortpulse_internal_billing_renewals_cron_secret
--      Example: same value as SHORTPULSE_INTERNAL_BILLING_RENEWALS_CRON_SECRET
-- Optional:
--   3) shortpulse_vercel_protection_bypass_token
--      Used only when target deployment is protected by Vercel auth.
--
-- This script auto-provisions the two renewal Vault secrets using existing recovery
...
--
-- URL derivation rule:
--   /api/internal/generation-recovery/run -> /api/internal/billing-contract-renewals/run
--
-- If recovery secrets are missing, create renewal secrets manually first or run the
...
begin
  select ds.id, ds.decrypted_secret
    into v_run_url_id, v_run_url
  from vault.decrypted_secrets ds
  where ds.name = 'shortpulse_internal_billing_renewals_run_url'
  ...
  from vault.decrypted_secrets ds
  where ds.name = 'shortpulse_internal_billing_renewals_cron_secret'
  order by ds.created_at desc
  limit 1;
  ...
  v_target_run_url := nullif(trim(coalesce(v_run_url, '')), '');
  if v_target_run_url is null then
    if nullif(trim(coalesce(v_recovery_run_url, '')), '') is null then
      raise exception
        'Missing Vault seed URL. Set shortpulse_internal_billing_renewals_run_url directly, or set shortpulse_recovery_run_url first.';
    end if;
  ...
  v_target_secret := coalesce(
    nullif(trim(coalesce(v_secret, '')), ''),
    nullif(trim(coalesce(v_recovery_secret, '')), '')
  );

  if v_target_secret is null then
    raise exception
      'Missing Vault seed secret. Set shortpulse_internal_billing_renewals_cron_secret directly, or set shortpulse_reconciler_cron_secret first.';
  end if;

  if v_run_url_id is null then
    perform vault.create_secret(
      v_target_run_url,
      'shortpulse_internal_billing_renewals_run_url',
      'ShortPulse internal billing renewals endpoint URL'
    );
  else
  ...
  if v_secret_id is null then
    perform vault.create_secret(
      ...
      'ShortPulse internal billing renewals cron bearer secret'
    );
  else
    perform vault.update_secret(
      v_secret_id,
...
begin
  for v_existing_job_id in
    ...
    from cron.job j
    where j.jobname in (
      'shortpulse_internal_billing_renewals_daily',
  ...
  select cron.schedule(
    'shortpulse_internal_billing_renewals_hourly',
    '15 * * * *',
    'select public.invoke_internal_billing_renewals_scheduler();'
  )
    into v_new_job_id;

  raise notice 'Scheduled job shortpulse_internal_billing_renewals_hourly with jobid=%', v_new_job_id;
end;
$$;
