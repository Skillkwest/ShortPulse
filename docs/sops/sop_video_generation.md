...
| Kie      | `kie-ai/seedance-2`, `kie-ai/seedance-2-fast` | 16:9 default (allowed: 1:1, 21:9, 4:3, 3:4, 16:9, 9:16) | Active Seedance 2 lanes route through `/api/fal/kie-seedance-2-submit` + `/api/fal/kie-seedance-2-status` and `/api/fal/kie-seedance-2-fast-submit` + `/api/fal/kie-seedance-2-fast-status`. Panel UX follows the Kling-pattern shot workspace and linked Character/Element surfaces, but submit compiles into Seedance-native prompt, frame, and multimodal reference payloads (`first_frame_url`, `last_frame_url`, `reference_*_urls`). Video settings expose duration `4-15` seconds; Seedance 2 supports `480p/720p/1080p`, Seedance 2 Fast supports `480p/720p`; audio defaults on; `return_last_frame` and `web_search` default off. Profile-bearing customer pricing is composition-neutral: references do not change credits at fixed model/resolution/output duration. Provider cost remains video-input-sensitive, and any video reference requires positive aggregate duration evidence no greater than 15 seconds before reservation/provider submit. Legacy profile-absent policies retain split customer rows for exact rollback. |
