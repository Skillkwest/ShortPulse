...
- `Libraries -> Media Library` is a first-class left-panel tool (`media-library`) in AI Studio and no longer modal-only by default. Runtime fallback: set `NEXT_PUBLIC_AI_STUDIO_MEDIA_LIBRARY_PANEL_ENABLED=false` to restore the legacy modal open path during rollback. Gesture/runtime gate: `NEXT_PUBLIC_AI_STUDIO_MEDIA_LIBRARY_GESTURE_V2_ENABLED` controls panel drag ghost + root right-click ingest + root delete semantics. Per-folder canvas spaces are always enabled for user-created folders, with independent camera/scene persistence through the active folder authority boundary: non-project surfaces use `/api/ai/media-folder-canvas/[folderId]` + `/api/ai/media-folder-canvas/save`, while project routes use `GET|PUT /api/projects/:projectId/media/folders/:folderId/canvas`. Target UX defines `All Media` as one master root folder with inline tabs (`All Media`, `Images`, `Videos`, `Prompts`) that organize content in-place. The mixed `All Media` tab now carries saved images, videos, audio, and prompts together, while audio also supports save/upload/root-drop/folder-drop flows through the same Media Library surface instead of a dedicated `Audio` tab. Near-bottom infinite scroll auto-load remains on media tabs with a global root media paginator footer retained as manual fallback, right-click media dispatch to Reference Grid, double-click preview-only modal behavior, and explicit folder membership move/assign semantics.
- `Libraries -> Elements` is a first-class left-panel tool (`elements`) in AI Studio. `Manage Elements` remains the only persistent panel surface, and selecting or creating an element opens the editor in a dedicated right-side column inside the Elements panel. New elements stage locally and only persist after `Save Element`, while existing saved elements continue to autosave in the editor.
- `Libraries -> Presets` opens one primary left-panel Presets Library with internal `Pulses` and `Prompt Presets` sections. The `Pulses` section is backed by shared per-user Pulse persistence. Clicking any Pulse opens the editor: built-in starter Pulses save as personal overrides on their seeded ids, while custom Pulses remain user-created records that can also be deleted. Guided `workflow_gpt` Pulses now persist authoritative workflow session state (`status`, current step, collected inputs, final artifact, completion source) so Create restore and the in-chat workflow banner do not depend on transcript-only reconstruction.
- Custom Pulse authoring now centers on `Preset Name` and hidden `System Instructions`; advanced runtime controls remain optional behind advanced settings, and newly created custom Pulses default to GPT-style start-immediately guided behavior.
- In Expert Create `Pulse` mode, the inline left rail uses the same shared Pulse catalog with pinned preset chips, drag/drop from `More Presets`, editable saved custom pulses, and hidden preset activation that updates the active Pulse runtime without writing into the visible Create composer. `More Presets` also acts as an activation surface and pins the selected Pulse automatically, while `Libraries -> Presets -> Pulses` remains catalog-management only. When a Pulse is active, the Create topbar now exposes an explicit `Deactivate Pulse` action, the in-chat session card presents `Active Pulse` / `Guided Pulse` copy instead of prompt-preset terminology, leaving Pulse mode deactivates Pulse runtime completely, and switching, restarting, or deactivating a Pulse starts a fresh guided session.
...
- `Sound -> Voice` keeps voice management inside the dedicated Voices panel: the top library header owns `Create New Voice` plus selected-voice deletion, backed by `GET /api/elevenlabs/voices` for hydration and `DELETE /api/elevenlabs/voices/:voiceId` for saved/provider cleanup.
- `Sound -> Voice -> Voice Changer` uses one storage-first preparation flow in both local and deployed environments, but the initial staging adapter now differs by source type: dropped local video is written through `POST /api/upload-video` so the original clip lands on the repo's stable storage-only adapter before extraction, while dropped local audio is written through `POST /api/media/stage-voice-changer-source`. Video sources are then converted into staged WAV audio through `POST /api/media/extract-audio`, and final conversion submits the staged audio reference to `POST /api/elevenlabs/speech-to-speech`. When that staged audio came from video, the final generation lane hard-locks to the current ElevenLabs default speech-to-speech voice settings (`stability: 1`, `similarity_boost: 1`, `speed: 1`, `use_speaker_boost: true`) and hides the Voice Shaping panel instead of exposing per-run slider tuning, then remuxes the converted voice back onto the original video and publishes that derived video as a second Reference Grid output. Extraction rejects trusted-but-oversized staged videos before ffmpeg runs so local and Vercel follow the same bounded path. Compatibility local uploads remain accepted on the final generation route only as a fallback for legacy callers.
...
- Video policy invariant:
  ...
  - Standard Video mode is Kie-only and surfaces `kie-ai/veo-3.1-fast-i2v`, `kie-ai/kling-3.0`, `kie-ai/seedance-1.5-pro`, `kie-ai/seedance-2`, and `kie-ai/seedance-2-fast`.
...
| fal-ai/bytedance/seedream/v5/lite/text-to-image | Aspect: 1:1 default (allowed: 1:1, 2:3, 3:2, 3:4, 4:3, 4:5, 5:4, 9:16, 16:9, 21:9); Resolution: auto_2K default (allowed: auto_2K, auto_3K) | Fal text-to-image queue; per-image pricing at $0.035 with shared credit rounding (5 billed credits); provider safety checker enabled. |
