...
Authority: Working  
Owner: Engineering  
Status: superseded for billing reservation details by `docs/planning/credit-grant-lot-expiration-buildout-plan-2026-07-06.md`

## Purpose

This document inventories the current request, attempt, output, and billing identities used by the generation pipeline.

Billing reservation authority has changed since this matrix was written. The runtime path must use the grant-lot reservation system and must not use direct-debit, legacy ledger, or aggregate-balance fallback billing paths.
...
| Billing reservation identity       | Grant-lot reservation allocations keyed by source correlation and reservation id                                                                     | `frontend/lib/server/api/generationBilling.ts` via grant-lot reservation RPCs                                                                                               | Billing reservation submit/capture/release in `generationBilling.ts` and `generationBilling/settlementService.ts`  | Submit path, queue dispatch, recovery settlement, admin diagnostics                      | Reservation must not be rebuilt from aggregate balance or legacy ledger rows               | Attach reservation to canonical request immediately; remove repair-first linkage model     |
