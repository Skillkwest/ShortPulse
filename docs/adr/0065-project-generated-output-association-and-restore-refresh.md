...
4. On project workspace reads, return the sanitized saved snapshot without blocking on generated-output projection/media refresh.
5. After project workspace bootstrap settles in AI Studio, refresh generated-output delivery asynchronously from generation rows that are explicitly associated to that project through `project_generation_items`.
6. Keep the current AI Studio snapshot envelope as the restore payload contract; this ADR changes generated-output authority, not the snapshot schema.

## Consequences

- Positive:
  - Project reopen now has a project-owned seam for generated outputs, not just saved media and prompts.
  - Workspace read no longer pays generated-output projection/media refresh before first paint.
  - Post-bootstrap generated-output refresh still uses project-owned membership instead of scanning all user-global generation rows.
