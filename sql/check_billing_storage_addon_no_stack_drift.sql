...
eligibility as (
    select
        ...
        case
            ...
            when storage_addon_id in ('storage_10gb', 'storage_25gb') then 'retired_legacy'
            when plan_id = 'starter' and storage_addon_id = 'storage_50gb' then 'eligible'
            when plan_id = 'media' and storage_addon_id in ('storage_50gb', 'storage_100gb') then 'eligible'
            when plan_id = 'studio' and storage_addon_id in ('storage_50gb', 'storage_100gb', 'storage_250gb') then 'eligible'
            when plan_id = 'business' and storage_addon_id in ('storage_50gb', 'storage_100gb', 'storage_250gb', 'storage_1tb') then 'eligible'
            when storage_addon_id in ('storage_50gb', 'storage_100gb', 'storage_250gb', 'storage_1tb') then 'plan_ineligible'
