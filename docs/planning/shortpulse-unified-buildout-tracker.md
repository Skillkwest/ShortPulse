...
| 08 | In Progress | Engineering | Phase 07 complete | Stripe idempotency/recovery correctness proven | `evidence/unified-buildout/phase-08/` | Slice A complete: webhook event claim flow converted to insert-first idempotent claim semantics with duplicate conflict + non-conflict claim failure handling before side effects. Slice B replay safety landed: duplicate-claim deliveries now continue side effects and ledger duplicate source-ref conflicts are treated as idempotent no-op for retry safety. Remaining Slice B work: billing bootstrap edge handling in checkout/portal routes. |
