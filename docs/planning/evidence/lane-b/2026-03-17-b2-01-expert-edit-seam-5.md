# Lane B Evidence Packet: B2-01 Expert Edit Seam 5
...
## Scope
1. Extract layer/session initialization and layer-stack normalization helpers out of `ExpertEditPanelView`.
...
4. `docs/planning/evidence/lane-b/2026-03-17-b2-01-expert-edit-seam-5.md`
...
1. `ExpertEditPanelView.tsx`: `5690` -> `5455` (`-235` lines, size-budget script count).
2. New module: `expertEditLayerSessionUtils.ts` (`271` lines).
3. Coupling reduction:
   - Layer naming/auto-name helpers moved into dedicated helper module.
   - Layer stack invariants and lowest-unused-layer-number helpers moved into dedicated helper module.
   - Session-layer hydration/normalization helpers and initial session state bootstrap moved into dedicated helper module.

## Parity Assertions
1. `ExpertEditPanelView` prop contract remained unchanged.
2. No API/server/schema changes.
3. No behavior changes to layer stack normalization, session restore, or markup/inpaint history bootstrap (utility extraction only).
...
## Rollback Note
1. Revert this slice commit to inline layer/session helper block back into `ExpertEditPanelView`.
