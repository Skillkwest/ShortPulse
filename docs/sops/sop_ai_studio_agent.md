...
| `frontend/pages/api/ai/studio-agent-standard.ts` / `frontend/pages/api/ai/studio-agent-pulse.ts`                                                                                                                                                                                      | Mode-owned API routes. Standard is a raw OpenAI pass-through for user/assistant turns with lane-boundary validation only, and that raw contract is intentionally frozen until a future dedicated change. Pulse keeps the guided workflow, request guards, and structured runtime behavior. |
