...
5. On project routes, disable browser-global workflow-settings persistence, chat-mode local storage persistence, and selected-character local storage persistence so those fields restore from project workspace state instead.
6. Keep plain `/ai-studio` routes without `projectId` on the legacy `sid` path until later migration phases complete.
