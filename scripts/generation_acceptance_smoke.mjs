...
const CASES = {
  ...
  "fal-nano-banana-text-image": {
    provider: "fal",
    modelId: "fal-ai/nano-banana",
    submitPath: "/api/fal/nano-banana-submit",
    statusPath: "/api/fal/nano-banana-status",
    timeoutMs: 300_000,
    pollEveryMs: 5_000,
    payload: ({ prompt, projectId }) => ({
      prompt,
  ...
  "fal-nano-banana-edit": {
    ...
    submitPath: "/api/fal/nano-banana-edit-submit",
    statusPath: "/api/fal/nano-banana-edit-status",
    timeoutMs: 300_000,
    pollEveryMs: 5_000,
  ...
  "fal-flux-pro-fill": {
    ...
    submitPath: "/api/fal/flux-pro-fill-submit",
    statusPath: "/api/fal/flux-pro-fill-status",
    timeoutMs: 300_000,
    pollEveryMs: 5_000,
    payload: async ({ prompt, projectId, resolveImageAsset }) => ({
      prompt,
      image_url: await resolveImageAsset("reference"),
      mask_url: await resolveImageAsset("mask"),
      num_images: 1,
      output_format: "png",
      safety_tolerance: "5",
      ...(projectId ? { shortpulse_context: { project_id: projectId } } : {}),
  ...
  "fal-flux-kontext-inpaint": {
    ...
    submitPath: "/api/fal/flux-kontext-inpaint-submit",
    statusPath: "/api/fal/flux-kontext-inpaint-status",
    timeoutMs: 300_000,
    pollEveryMs: 5_000,
    payload: async ({ prompt, projectId, resolveImageAsset }) => ({
      prompt,
      image_url: await resolveImageAsset("reference"),
      mask_url: await resolveImageAsset("mask"),
      reference_image_url: await resolveImageAsset("secondary-reference"),
      num_images: 1,
      output_format: "png",
      ...(projectId ? { shortpulse_context: { project_id: projectId } } : {}),
    }),
