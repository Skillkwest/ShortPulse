...
export default function AdminUserHealthFleetPage() {
  ...
  return (
    <AdminRouteShell
      ...
      documentTitle="ShortPulse · Admin Fleet Health"
      metaDescription="Fleet-level generation and drainage diagnostics for active users."
      pageTitle="Fleet health diagnostics"
      pageDescription="Hourly triage for active users with drill-down to per-user health and traces."
      userEmail={user?.email}
      currentPath="/admin/user-health-fleet"
    >
          ...
          <div className={styles.adminCard}>
            ...
            <p className={styles.adminSubtext}>
              ...
              {report.run.drainageTrend.previousRunId ? (
                <>
                  {" "}
                  · Δ released {formatSignedNumber(report.run.drainageTrend.releasedDelta ?? 0)} · Δ
                  errors {formatSignedNumber(report.run.drainageTrend.errorsDelta ?? 0)}
                </>
              ) : null}
            </p>
          </div>
