...
describe("pricingAnalysis", () => {
  ...
  it("scales shared-policy image costs linearly when amount changes", () => {
    ...
    expect(liveRow?.providerCostUsd).toBeCloseTo(0.08, 6);
    expect(draftRow).toMatchObject({
      usageLabel: "Amount",
      usageValueLabel: "3",
      creditsAtCost: 24,
    });
    expect(draftRow?.providerCostUsd).toBeCloseTo(0.24, 6);
  });
  ...
  it("expands Seedance 2.0 into customer-billed resolution price variants", () => {
    const pricingPolicy = getDefaultModelPricingPolicyDocument();
    const model = buildModelRow({
