...
- Symptom: `All Media` `generations_images` cards fill slowly in AI Studio panel.
  ...
  - Treat replay exit code `2` as “bounded run stopped at safety cap before claims drained”; inspect the final summary in `/tmp/media_derivative_backlog_replay/combined.log` and `/tmp/media_derivative_backlog_replay/final_summary.json` before increasing the cycle cap.
