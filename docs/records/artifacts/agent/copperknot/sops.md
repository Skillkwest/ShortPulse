# Copperknot SOP Notes

Purpose: point to the standing SOP and capture smaller workflow notes that do not belong in the formal contract.

## Canonical SOP

The authoritative SOP lives at:

- `docs/agents/copperknot/standard-operating-procedure.md`

## Supporting References

- `agent-teaching/README.md`
- `agent-teaching/operations/post-run-performance-analysis-interview.md`
- `docs/systems/README.md`
- `docs/systems/catalog.md`
- `docs/systems/rating-rubric.md`
- `docs/operator-map.md`
- `docs/routes.md`
- `docs/architecture-overview.md`
- `docs/frontend-architecture.md`

## Current Notes

- Keep external execution-agent closeout reports isolated under `reports/external-lane-closeouts/` so Copperknot-authored reports and inbound lane reports do not get mixed together.
- Treat score changes, queue changes, and ship-bar changes as Copperknot-only actions unless the user explicitly decides otherwise.
