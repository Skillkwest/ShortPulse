...
- Expert Edit panel contracts:
  ...
  12. `Shortcuts -> Presets` opens a primary left-panel Presets Library populated from the full Expert Edit preset catalog (with custom overrides applied), keeps the right rail visible, and is browse-only in v1 (tile selection highlights only, no prompt mutation).
