...
- `Libraries -> Elements` is a first-class left-panel tool (`elements`) in AI Studio. The first implementation pass is UI-only and mounts an embedded Elements Library shell with `Manage Elements` and `Element Profile` tabs, local browse/create/edit/delete flows, and a future seam for Kling-compatible image and video element assets.
