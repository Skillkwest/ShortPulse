...
1. Staging canary:
   ...
   - optional baseline helper:
     - `node scripts/capture_phase04_canary_baseline.mjs --base-url "$SHORTPULSE_STAGING_BASE_URL" --bootstrap-token-from-supabase --reconciler-secret "$SHORTPULSE_FAL_RECONCILER_CRON_SECRET" --vercel-api-token "$VERCEL_API_TOKEN" --vercel-bypass-token "$SHORTPULSE_VERCEL_PROTECTION_BYPASS_TOKEN"`
...
3. If deployment protection is enabled, provide `--vercel-api-token` (or `VERCEL_API_TOKEN` / `SHORTPULSE_VERCEL_API_TOKEN`) when running baseline capture automation. Keep `--vercel-bypass-token` as optional fallback.
