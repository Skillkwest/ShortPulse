...
| `/api/media/list` | `POST` | Bearer (proxy + route) | Server-authoritative media listing with tab or media-kind filtering, optional folder filter (`all_items` virtual root or custom-folder membership scope), keyset pagination, and optional first-slice signed-preview hydration. | `frontend/pages/api/media/list.ts`, `frontend/features/media-library/logic/mediaQueryModel.ts` |
