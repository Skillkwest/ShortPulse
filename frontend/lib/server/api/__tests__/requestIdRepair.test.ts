...
describe("repairGenerationRequestIdFromReservation", () => {
  ...
  it("backfills request_id from generation_attempts", async () => {
  ...
  it("does not backfill request_id from reservations", async () => {
    const generationSelectBuilder = createGenerationSelectBuilder({
      data: {
        id: "gen-1",
        user_id: "user-1",
  ...
  it("returns missing provider request id instead of reading reservations", async () => {
    const generationSelectBuilder = createGenerationSelectBuilder({
      data: {
        id: "gen-1",
        user_id: "user-1",
  ...
  it("surfaces generation_attempt lookup errors", async () => {
    lookupLatestGenerationAttemptMock.mockResolvedValueOnce({
      data: null,
      error: { message: "attempt lookup unavailable" },
    });
    const generationSelectBuilder = createGenerationSelectBuilder({
      data: {
        id: "gen-1",
    ...
    await expect(
      ...
      sourceRef: "source-1",
      reason: "db_error",
      errorMessage: "attempt lookup unavailable",
    });
    expect(ensureAcceptedRunningGenerationAttemptMock).not.toHaveBeenCalled();
  });
});
...
describe("repairGenerationRequestIdsFromReservations", () => {
  ...
  it("skips non-canonical provider keys from the batch scan", async () => {
    const batchBuilder = {
      is: vi.fn(),
      order: vi.fn(),
      limit: vi.fn(async () => ({
