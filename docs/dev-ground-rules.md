...
24. **Fix the source, not around it.** When something is broken or incomplete, trace the owning path and fix the canonical implementation. Do not add workaround code, backup files, fallback systems that mask failures, legacy variants, duplicate routes/components, or parallel behavior to bypass the real issue. If compatibility, migration support, or a platform-required alternate transport is genuinely required, document why it exists, who owns removal or the durable authority contract, and keep it out of the normal path unless that contract makes it the canonical path.
