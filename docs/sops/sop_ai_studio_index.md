...
- `Sound -> Voice -> Create New Voice` now has two modal creation methods. `Generate Voice` preserves the existing Voice Design preview/select/save flow through `POST /api/elevenlabs/text-to-voice/design` and `POST /api/elevenlabs/text-to-voice/create`. `Clone Voice` is audio-only in v1: local audio samples stage through `POST /api/media/stage-voice-clone-source` under the caller's private `voice-clone/source-audio` namespace, while durable Reference Grid/internal audio can also reach clone creation through an existing trusted storage path. Clone mode requires inline permission confirmation, then submits the trusted `sourceStoragePath` to `POST /api/elevenlabs/voices/clone`, where ShortPulse validates and normalizes the audio into a provider-safe payload before the ElevenLabs instant voice cloning request. Short samples are allowed; there is no app-enforced minimum-duration gate for voice clone samples. Clone mode intentionally does not reuse Voice Changer storage namespaces or `speech-to-speech` generation routes.
