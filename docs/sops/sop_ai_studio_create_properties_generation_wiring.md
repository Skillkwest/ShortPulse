...
| Workflow plan access          | `frontend/lib/billing/aiStudioWorkflowEntitlements.ts`, `frontend/features/ai-studio/routes/AiStudioRouteApp.tsx`, `frontend/lib/server/api/aiStudioWorkflowPlanGuard.ts`, `frontend/lib/server/api/falSubmitProxy.ts`, `frontend/pages/api/elevenlabs/*`                                                                                                                                                                             | Lets baseline/free accounts explore AI Studio Video and Sound while blocking generation, replaces Starter-restricted rail buttons with `View plans`, and denies executable video/audio submits before billing reservation or provider dispatch. |
...
- Enforce workflow-plan access before dispatch for Video/Sound entry points. Baseline/free users may navigate into Video/Sound for exploration, but their generation entry points must show the shared `View plans` CTA and must not dispatch. Starter users are not allowed to navigate into Video/Sound from the left rail and should see rail-level `View plans` CTAs instead. The route-level server guard must reject restricted FAL video and ElevenLabs audio generation before billing reservation or provider dispatch.
