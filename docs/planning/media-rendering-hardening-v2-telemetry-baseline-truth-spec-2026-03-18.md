...
| --- | --- | --- | --- | --- |
| first card render timing | `frontend/features/ai-studio/components/MediaLibraryModal.tsx` | Trusted with scope limit | The modal emits first-card shell today; panel, file modal, panel preview modal, reference-grid, character-grid, and detail-modal do not | Restrict baseline timing signoff to the modal until replacement metrics or new emitters exist |
| first media paint timing | `frontend/features/media-library/hooks/useMediaSurfacePreviewRuntime.ts`, `frontend/features/ai-studio/components/MediaLibraryModal.tsx` | Trusted with scope limit | Modal and panel first-media timing exists today; file modal, panel preview modal, reference-grid, character-grid, and detail-modal do not | Restrict timing baseline to modal/panel and document exclusions explicitly |
...
| --- | --- | --- | --- |
| first card render timing | historical predecessor notes, `frontend/features/ai-studio/components/MediaLibraryModal.tsx` | `media-library-modal` | No parallel first-card emitter exists today for panel/file/detail/character/reference surfaces |
...
| open-to-first-media | historical predecessor notes, `frontend/features/ai-studio/components/MediaLibraryModal.tsx` | `media-library-modal` | The modal is the only current live surface with this timing pair |
