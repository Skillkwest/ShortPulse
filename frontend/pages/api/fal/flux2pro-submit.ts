/**
 * Proxies Fal.ai FLUX 2 PRO text-to-image submit requests.
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const upstream = await fetch(FAL_FLUX2PRO_SUBMIT_URL, {
      method: "POST",
      headers: {
    ...
    return res.status(500).json({ error: "Fal FLUX 2 PRO submit failed", detail: String(error) });
  } finally {
