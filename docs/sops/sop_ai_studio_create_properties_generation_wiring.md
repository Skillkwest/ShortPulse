...
- In the Video Lip Sync lane, state effects pin the hidden internal Lip Sync model while the panel presents product-only controls (`Character image`, `Voice audio`, resolution, and generic faster-generation control). Generate guardrails require a character image and ready voice audio, block uploading/failed/stale local audio, enforce known audio-duration limits by selected resolution, and the submit path stages both character image and voice audio through `/api/fal/upload-url` so Fal OmniHuman receives Fal CDN URLs.
