...
2. `OPENAI_API_KEY` is required for separate text/describe workflows documented in `docs/sops/sop_text_generation.md`. Direct OpenAI `gpt-image-2` Create/Edit image routes are retired and disabled.
...
- Model policy:
  ...
  - Direct OpenAI `gpt-image-2` is retired and disabled. If stale Character Mode state still references direct `gpt-image-2`, Create model mapping resolves away from the retired model before submit/pricing.
