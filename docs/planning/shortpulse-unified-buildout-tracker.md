...
| 11 | In Progress | Engineering | Phase 10 complete | Kie migration thresholds pass shadow/canary | `evidence/unified-buildout/phase-11/` | Slice A pre-cutover readiness packet added: shadow/canary template with explicit thresholds, observation windows, and promote/hold/rollback decision framing. Slice B engineering hardening is complete: provider-neutral canonical payload identity parsing (request/event/status aliases), provider-aware recovery probe/submit/status dispatch seams, provider-aware status/result payload parsing seam (`statusProviderPayload` + shared `providerKey`), provider-owned status topology resolver (`statusProviderTopology`) wired into recovery probing, provider-owned response-url probe dispatch wired into shared status/recovery runtime, provider-owned polling session policy extraction (`statusProviderPolling`), provider-owned status/result candidate selection policy (`statusProviderSelection`), provider-owned lifecycle + retry policy (`statusProviderPolicy`), queued-submit alignment to shared provider dispatch contracts, explicit Fal route inventory regression gate coverage (still passing on latest run), AI Studio selector fail-closed guard that keeps Kie models hidden by default, a dedicated Kie model-contract execution boundary (`kieModelContracts`) wired into submit dispatch with fail-closed unsupported-model enforcement, a dedicated Kie status/result contract boundary (`kieStatusContracts`) wired into shared payload/policy adapters, a dedicated Kie result media normalization boundary (`kieResultMediaContracts`) wired into recovery/webhook media extraction paths, and model-aware Kie status/response topology gating plus deterministic queued submit-target resolution error codes (`KIE_RUNTIME_DISABLED`, `KIE_MODEL_NOT_ALLOWLISTED`, `KIE_SUBMIT_TARGET_RESOLUTION_FAILED`) with mixed-provider queue coverage. Canonical no-regression gate scripts are now available: `npm -C frontend run test:phase11:fal-regression` and `npm -C frontend run validate:phase11:fal-regression`. Kie dark-path foundation is in progress (runtime flags + provider seams + fail-closed guards) with public Fal routes unchanged and cutover still OFF by default. Deferred checkpoints are schedule-gated and explicitly marked as deferred by schedule (not blocked by engineering readiness): canary-1 decision valid at/after `2026-03-02 18:46:07 UTC`; canary-2 decision valid at/after `2026-03-03 18:46:07 UTC`. |
| 12 | In Progress | Engineering | Phase 11 complete | Cleanup/decommission complete + two green cycles | `evidence/unified-buildout/phase-12/` | Slice A pre-cleanup inventory/checklist prepared now (flags, compatibility shims, adapters, docs refs) so cleanup execution can start immediately after canary/cutover signoff. |
