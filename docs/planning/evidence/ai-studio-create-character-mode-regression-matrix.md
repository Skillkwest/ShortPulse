...
| Create/Image + Character Mode ON + model picker open            | Only `seedream/edit`, `nano-banana-2/edit`, and `nano-banana-pro/edit` shown.                                                        |
| Toggle OFF -> ON from `seedream/text-to-image`                  | Model remaps to `seedream/edit`.                                                                                                     |
| Toggle OFF -> ON from `nano-banana-2`                           | Model remaps to `nano-banana-2/edit`.                                                                                                |
...
| Toggle ON -> OFF from `nano-banana-2/edit`                      | Model remaps to `nano-banana-2`.                                                                                                     |
...
| Character Mode ON submit with refs + Seedream Edit              | Submit uses composed prompt + canonical app-owned refs, with provider `image_urls` minted at submit time by the image handler route. |
| Character Mode ON submit with refs + Nano Banana 2 Edit         | Submit uses composed prompt + canonical app-owned refs, with provider `image_urls` minted at submit time by the image handler route. |
| Character Mode ON submit with refs + Nano Banana Pro Edit       | Submit uses composed prompt + canonical app-owned refs, with provider `image_urls` minted at submit time by the image handler route. |
