# Lane B Evidence Packet: B2-01 Expert Edit Seam 11
...
## Scope
1. Extract repeated window-timeout clear/null-reset logic from `ExpertEditPanelView` into `expertEditInteractionUtils`.
2. Keep toast timing, remove-background pending timeout, and inpaint-collapse timer behavior unchanged.
...
4. `docs/planning/evidence/lane-b/2026-03-17-b2-01-expert-edit-seam-11.md`
...
| --- | --- | --- |
| `test -- ExpertEditPanelView.test.tsx` | 0 | pass |
| `type-check` | 0 | pass |
...
1. `ExpertEditPanelView.tsx`: `5276` -> `5247` (`-29` lines, size-budget script count).
2. `expertEditInteractionUtils.ts`: `114` -> `123` (`+9` lines) with shared timeout-ref helper.
3. Coupling reduction:
   - Timeout clear/null-reset behavior moved to shared interaction utility.
   - Expert Edit orchestration keeps behavior while reducing duplicate timeout cleanup blocks.

## Parity Assertions
1. `ExpertEditPanelView` prop contract remained unchanged.
2. No API/server/schema changes.
3. Timer semantics for toast visibility/fade, remove-background pending, and inpaint-collapse remained unchanged; extraction-only seam.
...
## Rollback Note
1. Revert this slice commit to inline timeout clear/null-reset logic back into `ExpertEditPanelView`.
