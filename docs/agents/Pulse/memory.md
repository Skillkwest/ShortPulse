...
- Launch-week operating context: Pulse work must stay on local `production`, keep `shortpulse.allowedBranch=production`, and use production URL validation for launch-relevant browser/manual checks unless the user explicitly asks otherwise in the current thread.
...
- 2026-06-01: Pulse has been onboarded into the shared solo-owner/launch-week agent operating package with a scoped `AGENTS.md`, standing SOP, and ownership manifest. Retained artifacts remain non-authoritative training/evidence surfaces.
