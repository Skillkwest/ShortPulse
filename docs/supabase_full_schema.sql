...
-- Generation child relational ownership and least-privilege end state
-- (migration 224).
create unique index if not exists ux_ai_generations_id_user
    on public.ai_generations (id, user_id);
create unique index if not exists ux_media_files_id_user
...
alter table public.ai_generation_submit_queue
    drop constraint if exists ai_generation_submit_queue_generation_owner_fk;
alter table public.ai_generation_submit_queue
    add constraint ai_generation_submit_queue_generation_owner_fk
    foreign key (generation_id, user_id)
    references public.ai_generations (id, user_id) on delete cascade;

alter table public.generation_attempts
    drop constraint if exists generation_attempts_generation_owner_fk;
alter table public.generation_attempts
...
alter table public.ai_generation_outputs
    drop constraint if exists ai_generation_outputs_generation_owner_fk,
    drop constraint if exists ai_generation_outputs_attempt_owner_fk,
    drop constraint if exists ai_generation_outputs_media_owner_fk;
alter table public.ai_generation_outputs
...
alter table public.generation_publications
    drop constraint if exists generation_publications_generation_owner_fk,
    drop constraint if exists generation_publications_attempt_owner_fk,
    drop constraint if exists generation_publications_output_owner_fk,
    drop constraint if exists generation_publications_media_owner_fk;
alter table public.generation_publications
...
alter table public.generation_projection
    drop constraint if exists generation_projection_generation_owner_fk,
    drop constraint if exists generation_projection_attempt_owner_fk;
alter table public.generation_projection
...
begin
    if new.user_id is distinct from old.user_id
       or new.generation_id is distinct from old.generation_id then
        raise exception 'Generation child ownership is immutable.' using errcode = '42501';
    end if;
    return new;
end;
$$;
revoke all on function public.reject_generation_child_owner_reassignment()
    from public, anon, authenticated;
...
-- Migration 225: private upload staging and service-role-only transport intents.
-- Signing, provider admission, billing, and durable promotion remain outside this table.
