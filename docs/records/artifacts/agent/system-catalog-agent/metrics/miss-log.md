...
Purpose: record under-ranked blockers, over-trusted systems, weak reports, and other mistakes so the Catalog Agent can improve its future measurement and queue choices.
...
| `2026-05-15` | `timing miss` | `Catalog Agent review loop` | The first completed lane sat too long before Catalog Agent review. | `medium` | `docs/agents/system-catalog-agent/catalog-tool-health-metrics.md` | Added time-based learning logs and kept rerating lead time as a standing metric. | A good catalog can still underperform if review latency is too slow during prelaunch. |
