...
- AI Studio session persistence APIs: authenticated save/get/list routes under `/api/ai/sessions/*` provide server-side durable snapshot persistence (`POST /api/ai/sessions/save`, `GET /api/ai/sessions/:sid`, `GET /api/ai/sessions?limit=&cursor=`) behind `SHORTPULSE_AI_STUDIO_SESSIONS_API_ENABLED` (default enabled). Legacy client persistence requires the master opt-in `NEXT_PUBLIC_AI_STUDIO_LEGACY_SESSION_PERSISTENCE_ENABLED=true` and then uses `NEXT_PUBLIC_AI_STUDIO_SESSION_PERSISTENCE_ENABLED` (default enabled) plus scoped controls (`NEXT_PUBLIC_AI_STUDIO_SESSION_WRITE_SHADOW_ENABLED`, `NEXT_PUBLIC_AI_STUDIO_SESSION_REMOTE_SHADOW_ENABLED`, `NEXT_PUBLIC_AI_STUDIO_SESSION_RESTORE_SHADOW_ENABLED`, `NEXT_PUBLIC_AI_STUDIO_SESSION_RESTORE_APPLY_ENABLED`, `NEXT_PUBLIC_AI_STUDIO_SESSION_RESTORE_APPLY_AGENT_ENABLED`) so disabled mode is applied consistently across write, restore, and session-list paths.
...
1. Copy `frontend/.env.example` to `frontend/.env.local` (or export the values in your shell) and set:
   ...
   - for legacy AI Studio save/restore, set `NEXT_PUBLIC_AI_STUDIO_LEGACY_SESSION_PERSISTENCE_ENABLED=true`
