...
Accepted - 2026-05-28; superseded in part by ADR 0090 on 2026-06-03
...
4. legacy folder-canvas persistence at the time of cutover.
...
1. Canonical runtime folder tables are `media_folders`, `media_folder_media_items`, and `media_folder_prompt_items`.
2. Project routes must use the same folder CRUD, membership, and list APIs as non-project routes.
...
5. Imported folder ids are preserved so memberships can move without client-visible remapping.
...
3. Historical project folder-canvas snapshots may be imported into `media_folder_canvas_states` during migration 136 so older data is not stranded during the production cutover.
...
3. operator verification confirms imported global folder trees and memberships are present.
