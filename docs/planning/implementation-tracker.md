...
| STG-02 SQL/RPC Hardening (`028`) | In Progress | @sleepyseamonster | `frontend`, `security`, `sql_lint`, `conversation_state_hardening_gate` | added, not yet applied in env | SQL matrix + gate script ready | `docs/planning/evidence/sql/2026-02-20-stg-02-local-preflight.md`, `docs/planning/evidence/sql/2026-02-20-stg-02-staging-validation.md` | workflow bootstrap merged to `main` via PR `#28`; staging runs through `22242944019` executed in `warn` mode; connectivity/auth now pass, current blocker is staging schema drift: `public.prune_ai_agent_conversation_state_expired(integer)` missing (028 not fully applied) |
