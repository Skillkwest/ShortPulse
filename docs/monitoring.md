...
4. Operator retrieval:
   ...
   - `/api/admin/crashes` and `/admin/crashes` = browser session-health rows with derived stale-session status for likely freezes or ungraceful exits; `/api/admin/crashes-status` updates open/resolved/ignored review state
