...
- Enforce per-user isolation: RLS on `media_files` and other user-owned tables, plus private `media_library` bucket paths (`auth.uid()` scoped).
- Protected routes: `/profile`, `/report-issue`, `/ai-studio`, and `/admin`.
