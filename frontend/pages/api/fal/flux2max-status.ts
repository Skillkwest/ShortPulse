/**
 * Proxies Fal.ai FLUX 2 MAX status + result fetch.
 * Accepts { requestId }, fetches status first, then result when ready.
 */
import type { NextApiRequest, NextApiResponse } from "next";

const FAL_FLUX2MAX_STATUS_URL = "https://queue.fal.run/fal-ai/flux-2-max/requests";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const statusResp = await fetch(`${FAL_FLUX2MAX_STATUS_URL}/${requestId}/status`, {
      method: "GET",
    ...
    const resultResp = await fetch(`${FAL_FLUX2MAX_STATUS_URL}/${requestId}`, {
      method: "GET",
      headers: { Authorization: `Key ${apiKey}` },
      signal: controller.signal,
    });
    ...
    return res.status(500).json({ error: "Fal FLUX 2 MAX status failed", detail: String(error) });
  } finally {
