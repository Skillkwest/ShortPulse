...
9. Temporarily switching `Pulse -> Standard -> Pulse` preserves the hidden Pulse runtime for the current live session unless the user explicitly deactivates, restarts, or changes Pulses.
...
- The app sends a hidden activation turn to `/api/ai/studio-agent`.
...
Legacy metadata such as `prompt_editor`, `activate_only`, and `apply_prompt` may still be read as compatibility input, but they are not the active product contract.
...
6. The agent transport sends the kickoff turn to `/api/ai/studio-agent` using an isolated Pulse session namespace.
...
6. Keep custom Pulse authoring centered on `Name` plus `System Instructions`; advanced workflow metadata is secondary.
...
- Switch `Pulse -> Standard -> Pulse` and confirm the live hidden Pulse runtime is preserved only when no explicit deactivate/restart/change-Pulse action occurred.
...
3. Which state transitions are allowed to preserve the hidden Pulse runtime, and which must always reset it?
