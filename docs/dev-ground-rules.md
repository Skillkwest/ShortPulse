...
20. **Use the three-lane promotion path deliberately.** New work starts on local-only `working-development`, moves to hosted `staging-preview` for proof, and reaches `production` only after staging approval. Keep `shortpulse.allowedBranch` worktree-scoped and never push directly to `main`. See `docs/launch-week-production-operations.md`.
...
25. **Use the proof surface for the active lane.** Local implementation uses localhost and the development Supabase project; hosted testing uses the canonical staging Preview and staging project; production claims require `https://www.shortpulse.ai` and production evidence. Never treat local or staging proof as deployed production proof.
