...
- Chunk 5 (Kie model contract integration): in progress (dark-path model metadata scaffolding landed; submit-contract drift lock now sources Kie constraints from canonical model catalog; Kie model-id constants and provider header-boolean parsing are now centralized across provider contract boundaries; primary-source contract capture still required before enablement).
