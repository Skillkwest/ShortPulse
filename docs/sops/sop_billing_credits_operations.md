...
- `billing_plans` defines the shared plan ids, including the non-public baseline-access sentinel (`free`) plus the public paid tiers (`starter`, `media`, `studio`, `business`).
...
- Recurring storage add-ons are single-slot entitlements: one current billable add-on per user, quantity exactly one, and self-serve eligibility must come from `frontend/lib/billing/storageAddonEligibility.ts`. Self-serve changes may replace the one active add-on with a different eligible add-on; they must not stack multiple recurring storage items.
- `storage_10gb`, `storage_25gb`, and `storage_500gb` are retired from self-serve storage add-on eligibility and must not be exposed as public self-serve checkout options.
...
- Customer-facing subscription changes use `/api/billing/subscription/change`. Existing owned Stripe subscriptions must not create a second subscription through new Checkout. Single-item existing Stripe subscriptions use Stripe Billing Portal `subscription_update_confirm` for target-aware plan changes, including higher-plan upgrades, so the customer sees and confirms Stripe's charge before the subscription changes. Higher-plan upgrades must use the dedicated full-price Portal configuration from `STRIPE_BILLING_PORTAL_FULL_PRICE_UPGRADE_CONFIG_ID`; that configuration must set `features.subscription_update.proration_behavior = none`, `features.subscription_update.billing_cycle_anchor = now`, and `features.subscription_update.default_allowed_updates = ['price']`, so the customer pays the full target-plan price and starts a fresh billing period on the upgrade date. Local entitlement changes and credit grants must wait for Stripe to accept payment and emit the paid invoice/subscription webhooks. Subscriptions that also carry recurring storage add-on items are blocked from self-serve plan changes until the active storage add-on is removed or changed first, because Stripe Billing Portal does not support targeted multi-item subscription updates through this surface. The route validates the target Stripe price amount and interval against `billing_plan_offers` before creating Checkout/Portal sessions.
- Stripe Billing Portal configuration is part of the customer-facing subscription-change contract for single-item existing subscriptions. `features.subscription_update.products[*].prices` must include every active monthly and annual paid-plan price that `/api/billing/subscription/change` can target through Portal, and interval-shortening changes such as annual to monthly must be scheduled at period end through the portal `schedule_at_period_end` conditions to match the `/profile` downgrade language. The dedicated full-price upgrade configuration must be verified with `npm -C frontend run billing:portal-upgrade-config:verify` before production use. Portal update flows must not be used for subscriptions with recurring storage add-on items because Stripe rejects multi-item subscription updates through that surface.
- A same-target customer retry after failed authentication or an abandoned full-price Portal upgrade must recover Stripe's existing live pending update instead of replacing it. `/api/billing/subscription/change` may return the pending update's Hosted Invoice Page only after verifying that the update is unexpired, its subscription item and target price match the request, and its latest invoice belongs to the authenticated customer's verified subscription, is open with an amount remaining, and has a Stripe invoice URL. This recovery creates no new Portal session or local change intent. When no matching live pending update exists, the normal fresh Portal confirmation remains canonical. Do not reuse or persist short-lived Portal URLs or apply a long-lived idempotency key across customer retry attempts. Historical void invoices and PaymentIntents canceled with `cancellation_reason = 'void_invoice'` remain accurate retry history and must not be deleted.
- Stripe subscription schedules for period-end downgrades and interval-shortening changes are projected into `billing_subscription_scheduled_changes` by `subscription_schedule.*` webhooks. This projection is display/support state only: current entitlements and media access remain authorized by the open `billing_subscription_contracts` row until Stripe changes the active subscription item and the `customer.subscription.updated` webhook syncs the new contract terms.
- Customer-facing recurring storage changes use `/api/billing/storage-addon/change`. Adding a different eligible storage add-on updates the existing storage subscription item in Stripe, preserving the one-active-add-on invariant while avoiding a remove-then-add customer loop.
...
- `/admin` selected-account Status row stays collapsed by default but shows compact review pills when account warnings, errors, diagnostic findings, or pricing mismatches need attention.
...
- `npm -C frontend run billing:credits:verify -- --limit 25 --json` for read-only reconciliation of recent paid Stripe subscription allocation invoices against local `subscription_renewal` credit ledger and grant rows. This verifier detects paid `subscription_create`, `subscription_cycle`, legacy ShortPulse-marked immediate-upgrade `subscription_update`, and Portal-confirmed higher-plan `subscription_update` invoices that are missing the canonical `invoice:<invoice_id>:monthly_allocation` local grant path; it also compares local grant amounts against the invoice-time plan offer when the Stripe invoice line price can be resolved.
...
Use this sequence when `npm -C frontend run billing:credits:verify -- --json` reports `missing_credit_grant` for a paid subscription allocation invoice.

1. Confirm the Stripe invoice is paid and its `billing_reason` is `subscription_create`, `subscription_cycle`, or an eligible higher-plan `subscription_update` proven by either the legacy ShortPulse immediate-upgrade marker or invoice lines that resolve from the prior plan to a ranked-higher target plan.
...
6. Verify `/profile?section=subscription` still routes each plan card to the intended self-serve flow:
   - baseline-access/internal-comp to paid should open Stripe Checkout for the selected target plan

- Stripe-managed higher-plan upgrades should open Stripe Billing Portal confirmation for the selected target plan with the dedicated full-price/no-proration Portal configuration, then rely on the paid invoice webhook for the upgraded monthly credit allocation
- Stripe-managed downgrades and non-upgrade single-item changes should open a Stripe Billing Portal plan-change flow
...
- internal-comp back to baseline access should complete in-app and return the user to the subscription section
- pricing-page acquisition cancellations should return to `/pricing` with the selected
  plan/interval preserved plus `checkout=cancel`, not to protected account-management gates; pricing
  uses that marker to avoid rendering canceled Checkout returns as confirmed selected-plan state
...
- Subscription monthly credits are granted only for invoice payment events that represent a new billing allocation window (`billing_reason in ('subscription_create', 'subscription_cycle')`) or a paid, provable higher-plan `subscription_update`.
...
- Generic subscription change/proration invoices must not mint an extra monthly credit grant. A `subscription_update` invoice may grant the target monthly allocation only when it is paid and either carries the legacy ShortPulse immediate-upgrade marker, its invoice lines resolve to a positive target plan offer ranked above the negative prior-plan proration line, or it has a matching service-role-only `billing_subscription_change_intents` row for a full-price upgrade plus a full target-plan invoice line that matches the catalog recurring price. Full-price intent validity is evaluated against Stripe's immutable `invoice.created` time, not webhook receipt or eventual payment time, so delayed payment and replay remain recoverable only when Stripe created the invoice inside the customer-authorized intent window.
...
Covered AI Studio image, video, and audio submits carry the displayed policy version and billed credits. `chargeGenerationRequest` compares them with the active canonical result before reservation. `PRICING_POLICY_VERSION_REQUIRED` and `PRICING_POLICY_STALE` are safe pre-provider rejections: refresh pricing, preserve the customer's inputs, and require a new explicit Generate click. Do not treat these conflicts as debit failures or retry them automatically. Admin mismatch totals count distinct `source_ref` runs while the reservation and ledger row totals remain raw coverage counts.
...
- Accounts in baseline access or managed internally must not silently self-grant recurring storage add-ons. They should be blocked with explicit product messaging until a paid Stripe-managed subscription exists.
