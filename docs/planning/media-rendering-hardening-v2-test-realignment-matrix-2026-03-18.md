...
| `frontend/features/ai-studio/components/__tests__/MediaLibraryPanel.test.tsx` + `frontend/features/ai-studio/components/__tests__/MediaLibraryAllItemsGrid.test.tsx` | Panel keeps panel-specific preview-resolution wiring and routes signed poster/image inputs through the adaptive preview callback without inheriting route/modal surface semantics | Keep | Extend to assert the accepted panel policy rather than treat compatibility handling as target behavior |
