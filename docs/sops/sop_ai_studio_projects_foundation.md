...
8. `PUT` enforces the Reference Grid visible-active cap and prunes terminal hidden rows before durable checkpoint/display work. Overflow visible rows are not retained in the hot project workspace snapshot; operators should use the emitted over-cap telemetry and global media/generation authority when repairing already-over-cap projects.
