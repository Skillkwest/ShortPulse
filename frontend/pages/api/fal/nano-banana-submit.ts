/**
 * Proxies Fal.ai Nano Banana text-to-image submit requests.
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const upstream = await fetch(FAL_NANO_BANANA_SUBMIT_URL, {
      method: "POST",
      headers: {
    ...
    return res.status(500).json({ error: "Fal Nano Banana submit failed", detail: String(error) });
  } finally {
