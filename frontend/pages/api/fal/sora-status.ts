/**
 * Proxies Fal.ai Sora 2 Pro text-to-video status + result fetch.
 * Accepts { requestId }, returns status as-is, and fetches the result when completed.
 */
import type { NextApiRequest, NextApiResponse } from "next";

const FAL_SORA_STATUS_URL = "https://queue.fal.run/fal-ai/sora-2/requests";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const statusResp = await fetch(`${FAL_SORA_STATUS_URL}/${requestId}/status`, {
      method: "GET",
    ...
    if (!contentType || !contentType.includes("application/json")) {
      ...
      return res.status(500).json({
        error: "Fal Sora returned non-JSON response",
    ...
    const resultResp = await fetch(`${FAL_SORA_STATUS_URL}/${requestId}`, {
      method: "GET",
      headers: { Authorization: `Key ${apiKey}` },
      signal: controller.signal,
    });
    ...
    return res.status(500).json({ error: "Fal Sora status failed", detail: String(error) });
  } finally {
