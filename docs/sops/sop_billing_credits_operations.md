...
- `npm -C frontend run billing:launch-readiness` for a read-only production launch-readiness audit of billing-critical Vercel routes/env, signup callback origin, public pricing catalog, production Supabase billing catalog/linkage, the billing renewal worker fail-closed boundary, and optional DB-trigger/Stripe-webhook proof when read credentials are available.
