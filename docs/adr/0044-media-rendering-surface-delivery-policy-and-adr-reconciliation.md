...
Runtime policy note (2026-05-30): [ADR 0087](./0087-supabase-image-transformation-prohibition.md) constrains this ADR. Any historical transform-backed compatibility or migration wording below is superseded for runtime policy. Supabase image transformations are prohibited on all paths.
...
- Existing transform-backed preview URLs are retired migration history, not an allowed runtime compatibility lane.
...
- Supabase render-image transform URLs are prohibited and any runtime appearance is a regression.
...
- and the rule that preview hot paths must not depend on Supabase transforms.
...
- Positive:
  ...
  - Media Library and Reference Grid keep the strategies they already rely on, with durable previews and transform-free delivery as the accepted direction.
...
- Negative:
  ...
  - Some existing transform-capable paths now become explicit removal debt instead of accepted end state.
- Follow-ups:
