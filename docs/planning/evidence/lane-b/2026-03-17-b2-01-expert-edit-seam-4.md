# Lane B Evidence Packet: B2-01 Expert Edit Seam 4
...
## Scope
1. Extract inpaint/markup cursor-reticle data-URI builders out of `ExpertEditPanelView`.
...
2. `frontend/features/ai-studio/components/edit/expertEditCursorUtils.ts`
3. `docs/planning/evidence/lane-b/README.md`
4. `docs/planning/evidence/lane-b/2026-03-17-b2-01-expert-edit-seam-4.md`
...
1. `ExpertEditPanelView.tsx`: `5740` -> `5690` (`-50` lines, size-budget script count).
2. New module: `expertEditCursorUtils.ts` (`67` lines).
3. Coupling reduction:
   - Inpaint brush reticle cursor builder moved into dedicated helper.
   - Markup brush reticle cursor builder moved into dedicated helper.
   - Inpaint lasso cursor builder moved into dedicated helper.

## Parity Assertions
1. `ExpertEditPanelView` prop contract remained unchanged.
2. No API/server/schema changes.
3. No behavior changes to cursor visuals or cursor-application paths (utility extraction only).
...
## Rollback Note
1. Revert this slice commit to inline cursor-reticle builders back into `ExpertEditPanelView`.
