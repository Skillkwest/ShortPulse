...
- `npm -C frontend run gear-ball:preflight -- --files <paths...> --tests <tests...>`
  ...
  - Normalizes repo-root `frontend/...` Vitest paths to frontend-relative form, can print a frontend-relative test manifest with `--print-test-manifest`, and accepts newline-delimited file/test manifests via `--files-from` and `--tests-from`.
