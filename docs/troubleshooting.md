...
- Preferred guarded replay path:
  ...
  - The replay helper captures before/after backlog + terminal diagnostics when `SUPABASE_DB_URL` is set, logs per-cycle worker summaries (`triggerSource`, `durationMs`, `claimed`, `ready`, `failed`, `retryScheduled`, `exhausted`, `errors`), writes a machine-readable final summary at `/tmp/media_derivative_backlog_replay/final_summary.json` by default, and exits non-zero if the bounded cycle cap is reached before claims drain.
