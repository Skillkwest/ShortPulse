...
- Use `Copy triage` or `Copy all new` in the Incidents table for grouped-incident handoff packets, and `Copy triage` in the Event Stream table for single-event handoff packets. These payloads are versioned and intentionally compact (key identifiers + normalized triage metadata) to keep troubleshooting reproducible without pasting full raw metadata blobs.
