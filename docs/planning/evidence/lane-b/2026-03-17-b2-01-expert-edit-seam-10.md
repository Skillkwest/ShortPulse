# Lane B Evidence Packet: B2-01 Expert Edit Seam 10
...
## Scope
1. Extract generation-mode to rail-tool mapping and inpaint-collapse toggle decision logic from `ExpertEditPanelView` into `expertEditInteractionUtils`.
2. Keep rail selection and collapse-toggle behavior unchanged.
...
4. `docs/planning/evidence/lane-b/2026-03-17-b2-01-expert-edit-seam-10.md`
...
1. `ExpertEditPanelView.tsx`: `5278` -> `5276` (`-2` lines, size-budget script count).
2. `expertEditInteractionUtils.ts`: expanded with pure rail/collapse decision helpers.
3. Coupling reduction:
   - Generation-mode rail mapping moved into interaction utility.
   - Inpaint collapse toggle branch-decision logic moved into interaction utility.

## Parity Assertions
1. `ExpertEditPanelView` prop contract remained unchanged.
2. No API/server/schema changes.
3. Generation-mode rail selection and inpaint collapse-toggle behavior remained unchanged; extraction-only seam.
...
## Rollback Note
1. Revert this slice commit to inline rail/collapse decision logic back into `ExpertEditPanelView`.
