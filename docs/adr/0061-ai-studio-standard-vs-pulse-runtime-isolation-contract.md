...
6. Switching from `Pulse` to `Standard` clears active Pulse runtime ownership for the live session. Returning to `Pulse` starts with no active Pulse until the user explicitly starts one.
...
2. Existing UI flows that previously preserved adjacent state need clearer lifecycle boundaries around mode switching, activation, restart, and explicit Pulse deactivation.
