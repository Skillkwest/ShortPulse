...
- Approved panel mixed-open baselines now include a bounded `/api/media/list` seed on the first page with empty query (`up to five` non-audio preview URLs, matching the list-route cap) before the follow-up lazy sign pass. Treat that seed as part of the canonical hot path, not as anomalous preloading.
