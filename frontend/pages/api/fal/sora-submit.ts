/**
 * Proxies Fal.ai Sora 2 Pro text-to-video submit requests.
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const upstream = await fetch(FAL_SORA_SUBMIT_URL, {
      method: "POST",
      headers: {
    ...
    return res.status(500).json({ error: "Fal Sora submit failed", detail: String(error) });
  } finally {
