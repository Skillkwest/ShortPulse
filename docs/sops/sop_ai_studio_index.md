...
- `Sound -> Voice` keeps voice management inside the dedicated Voices panel: the header owns a `Voices` modal trigger plus `Create New Voice` and selected-voice deletion, backed by `GET /api/elevenlabs/voices` for hydration and `DELETE /api/elevenlabs/voices/:voiceId` for saved/provider cleanup. The selectable voice chip library now lives inside that modal instead of the main panel body.
