...
- Approved panel mixed-open baselines now include a bounded `/api/media/list` seed on the first page with empty query (`up to two` initial signed preview/playback URLs for panel surfaces) before the follow-up lazy sign pass. Treat that seed as part of the canonical hot path, not as anomalous preloading.
...
- Retained attribution is surface-specific: AI Studio uses `media-library-panel`, while Elements uses `elements-media-panel`. Both still share the same panel image-card preview signing profile; this profile is telemetry/cache compatibility only and must not enable Supabase image transforms.
