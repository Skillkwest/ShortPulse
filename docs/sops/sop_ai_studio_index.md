...
- `Libraries -> Pulse Presets` opens a primary left-panel Pulse Presets Library backed by shared per-user Pulse persistence. Clicking any Pulse opens the editor: built-in starter Pulses save as personal overrides on their seeded ids, while custom Pulses remain user-created records that can also be deleted.
...
- `Sound -> Voice -> Voice Changer` uses one canonical source-staging path in both local and deployed environments: dropped local audio/video is written directly to private storage, video sources are converted into staged WAV audio through `POST /api/media/extract-audio`, and final conversion submits the staged audio reference to `POST /api/elevenlabs/speech-to-speech`. Extraction rejects trusted-but-oversized staged videos before ffmpeg runs so local and Vercel follow the same bounded path. Compatibility local uploads remain accepted on the final generation route only as a fallback for legacy callers.
